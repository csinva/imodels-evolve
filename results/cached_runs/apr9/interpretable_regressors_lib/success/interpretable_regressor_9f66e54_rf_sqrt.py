...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # Tiny GBM correction: only 20 stumps (depth=1), very conservative
        ...
        if resid_frac > 0.20:  # Very conservative
            from sklearn.ensemble import RandomForestRegressor as _RFR
            self.tiny_gbm_ = _RFR(n_estimators=100, max_depth=4, max_features='sqrt', min_samples_leaf=5, random_state=42)
            self.tiny_gbm_.fit(X, resid)
        else:
            self.tiny_gbm_ = None
    ...
    def predict(self, X):
        ...
        if hasattr(self, 'tiny_gbm_') and self.tiny_gbm_ is not None:
            pred += self.tiny_gbm_.predict(X)
        return pred
...
model_shorthand_name = "SmoothGAM_RF_sqrt"
model_description = "SmoothGAM + RF 100 trees depth-4 sqrt features on residuals"
