...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if resid_frac > 0.20:  # Very conservative: only when >20% variance remains
            self.tiny_gbm_ = _GBR(n_estimators=20, max_depth=1, learning_rate=0.05,
                                   random_state=42)
...
model_shorthand_name = "SmoothGAM_tinyGBM"
model_description = "SmoothAdditiveGAM + tiny GBM (20 stumps, only when >20% residual var)"
