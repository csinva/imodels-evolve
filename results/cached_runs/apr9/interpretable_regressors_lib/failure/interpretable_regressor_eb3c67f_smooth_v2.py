...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        for j in range(n_features):
            ...
            if len(intervals) > 2:
                ...
                for _ in range(5):  # 5 passes of smoothing
                    new_intervals = [smooth_intervals[0]]
                    for k in range(1, len(smooth_intervals) - 1):
                        new_intervals.append(
                            0.5 * smooth_intervals[k] +
                            0.25 * smooth_intervals[k - 1] +
                            0.25 * smooth_intervals[k + 1]
                        )
    ...
    def predict(self, X):
        ...
        # Use full piecewise for ALL features (best prediction accuracy)
        for j, (thresholds, intervals) in self.shape_functions_.items():
            xj = X[:, j]
            bins = np.digitize(xj, thresholds)
...
model_shorthand_name = "SmoothGAM_v2"
model_description = "SmartAdditiveGAM + stronger Laplacian smoothing (5 passes, 0.5/0.25/0.25)"
