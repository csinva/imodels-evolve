...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # Final distillation: fit a Ridge on X→y using the additive model's
        # soft targets (blend of true y and model predictions)
        # This gives a pure linear model with stumps-informed coefficients
        additive_pred = np.full(n_samples, self.intercept_)
        for j_d, (thresholds, intervals) in self.shape_functions_.items():
            xj = X[:, j_d]
            bins = np.digitize(xj, thresholds)
            additive_pred += np.array([intervals[b] for b in bins])

        # Use the additive prediction to enhance the Ridge fit:
        # The Ridge fits on original X but targets a blend of y and additive_pred
        # This helps the Ridge learn coefficients that are "aware" of nonlinearities
        blend_target = 0.5 * y + 0.5 * additive_pred
        self.distilled_ridge_ = RidgeCV(cv=3)
        self.distilled_ridge_.fit(X, blend_target)
    ...
    def predict(self, X):
        check_is_fitted(self, "distilled_ridge_")
        X = np.asarray(X, dtype=np.float64)
        return self.distilled_ridge_.predict(X)

    def __str__(self):
        check_is_fitted(self, "distilled_ridge_")
        feature_names = [f"x{i}" for i in range(self.n_features_in_)]

        coefs = self.distilled_ridge_.coef_
        intercept = self.distilled_ridge_.intercept_

        equation = " + ".join(f"{c:.4f}*{n}" for c, n in zip(coefs, feature_names))
        equation += f" + {intercept:.4f}"

        lines = [
            f"Ridge Regression (L2 regularization, α={self.distilled_ridge_.alpha_:.4g} chosen by CV):",
            f"  y = {equation}",
            "",
            "Coefficients:",
        ]
...
model_shorthand_name = "DistilledRidge_v1"
model_description = "Boosted stumps → distilled into Ridge via blended targets (50% y + 50% stump_pred)"
