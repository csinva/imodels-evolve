...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # Phase 2: Extra boosting on nonlinear features to improve rank
        # Compute residuals after the hybrid predict
        hybrid_pred = np.full(n_samples, self.intercept_)
        total_imp = sum(self.feature_importances_)
        for j_p, (thresholds, intervals) in self.shape_functions_.items():
            slope, offset, r2 = self.linear_approx_.get(j_p, (0, 0, 0))
            if r2 > 0.70 and total_imp > 1e-10 and self.feature_importances_[j_p] / total_imp >= 0.01:
                hybrid_pred += slope * X[:, j_p] + offset
            else:
                xj = X[:, j_p]
                bins = np.digitize(xj, thresholds)
                hybrid_pred += np.array([intervals[b] for b in bins])

        residuals_phase2 = y - hybrid_pred
        # Only do extra boosting if significant variance remains
        if np.var(residuals_phase2) / np.var(y) > 0.05 if np.var(y) > 1e-10 else False:
            # Do 100 more rounds of boosting on residuals, only on nonlinear features
            nonlinear_js = [j_p for j_p in self.shape_functions_
                           if self.linear_approx_.get(j_p, (0, 0, 1.0))[2] <= 0.70
                           and self.feature_importances_[j_p] / total_imp >= 0.01]
            if nonlinear_js:
                for _ in range(100):
                    best_stump = None
                    best_reduction = -np.inf
                    for j_p in nonlinear_js:
                        xj = X[:, j_p]
                        order = np.argsort(xj)
                        xj_sorted = xj[order]
                        r_sorted = residuals_phase2[order]
                        cum_sum = np.cumsum(r_sorted)
                        total_sum = cum_sum[-1]
                        lo = self.min_samples_leaf - 1
                        hi = n_samples - self.min_samples_leaf - 1
                        if hi < lo:
                            continue
                        valid = np.where(xj_sorted[lo:hi + 1] != xj_sorted[lo + 1:hi + 2])[0] + lo
                        if len(valid) == 0:
                        ...
                        if reduction[best_idx] > best_reduction:
                            ...
                            left_mean = left_sum[best_idx] / left_count[best_idx]
                            right_mean = right_sum[best_idx] / right_count[best_idx]
                            best_stump = (j_p, threshold,
                                          left_mean * self.learning_rate,
                                          right_mean * self.learning_rate)
                    if best_stump is None:
                        break
                    j_p, threshold, left_val, right_val = best_stump
                    # Add to existing shape function
                    if j_p in self.shape_functions_:
                        old_thresholds, old_intervals = self.shape_functions_[j_p]
                        # Rebuild with the new stump
                        all_stumps = feature_stumps.get(j_p, [])
                        all_stumps.append((threshold, left_val, right_val))
                        feature_stumps[j_p] = all_stumps
                        new_thresholds = sorted(set(t for t, _, _ in all_stumps))
                        new_intervals = []
                        for i_int in range(len(new_thresholds) + 1):
                            if i_int == 0:
                                test_x = new_thresholds[0] - 1
                            elif i_int == len(new_thresholds):
                                test_x = new_thresholds[-1] + 1
                            else:
                                test_x = (new_thresholds[i_int - 1] + new_thresholds[i_int]) / 2
                            val = sum(lv if test_x <= t else rv for t, lv, rv in all_stumps)
                            new_intervals.append(val)
                        # Re-smooth
                        if len(new_intervals) > 2:
                            smooth = list(new_intervals)
                            for _ in range(3):
                                new_s = [smooth[0]]
                                ...
                                smooth = new_s
                            new_intervals = smooth
                        self.shape_functions_[j_p] = (new_thresholds, new_intervals)
                    mask = X[:, j_p] <= threshold
                    residuals_phase2[mask] -= left_val
                    residuals_phase2[~mask] -= right_val

                # Update feature importances for nonlinear features
                for j_p in nonlinear_js:
                    if j_p in self.shape_functions_:
                        _, intervals = self.shape_functions_[j_p]
                        self.feature_importances_[j_p] = max(intervals) - min(intervals)
...
model_shorthand_name = "SmoothGAM_boost2"
model_description = "SmoothAdditiveGAM + Phase 2 extra boosting on nonlinear features"
