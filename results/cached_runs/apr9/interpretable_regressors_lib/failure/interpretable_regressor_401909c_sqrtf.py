...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        for round_idx in range(self.n_rounds):
            ...
            # Feature subsampling: use sqrt(n_features) random features per round
            n_sub = max(1, int(np.sqrt(n_features)))
            feat_subset = rng.choice(n_features, n_sub, replace=False) if n_features > n_sub else np.arange(n_features)
...
model_shorthand_name = "SmoothGAM_sqrtF"
model_description = "SmoothAdditiveGAM with sqrt(n_features) random feature subsampling per round"
