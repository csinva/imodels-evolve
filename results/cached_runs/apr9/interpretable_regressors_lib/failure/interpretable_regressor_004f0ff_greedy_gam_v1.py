...
class GreedyGAMRegressor(BaseEstimator, RegressorMixin):
    """
    Greedy additive model with controlled per-feature complexity.

    Greedy gradient boosting with depth-1 stumps — always picks the globally
    best feature/threshold to split on (unlike cyclic approaches). Each feature
    is limited to max_splits_per_feature unique thresholds, keeping the
    display compact. Multiple boosting rounds can refine the values at
    existing thresholds without adding new ones.

    Model: y = intercept + f0(x0) + f1(x1) + ... + fp(xp)
    where each fi has at most max_splits+1 intervals.
    """

    def __init__(self, n_rounds=300, learning_rate=0.1,
                 min_samples_leaf=5, max_splits_per_feature=5):
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_rounds):
            ...
            for j in range(n_features):
                ...
                # If feature is at split limit, only allow existing thresholds
                at_limit = len(feature_splits[j]) >= self.max_splits_per_feature
                if at_limit:
                    existing = feature_splits[j]
                    # Filter valid to only positions near existing thresholds
                    midpoints = np.array([(xj_sorted[v] + xj_sorted[v + 1]) / 2 for v in valid])
                    mask = np.array([any(abs(m - t) < 1e-10 for t in existing) for m in midpoints])
                    valid = valid[mask]
                    if len(valid) == 0:
                        continue
...
GreedyGAMRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "GreedyGAM_v1"
model_description = "Greedy additive stumps with max 5 splits per feature, 300 rounds, lr=0.1"
model_defs = [(model_shorthand_name, GreedyGAMRegressor())]
