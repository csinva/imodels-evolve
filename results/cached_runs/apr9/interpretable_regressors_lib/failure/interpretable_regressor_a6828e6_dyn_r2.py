...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def _get_r2_threshold(self):
        """Dynamic R² threshold based on number of active features."""
        n_active = sum(1 for j in self.shape_functions_
                      if self.feature_importances_[j] > 0.01 * sum(self.feature_importances_))
        if n_active <= 5:
            return 0.60  # Few features: show more piecewise detail
        elif n_active <= 10:
            return 0.70  # Medium: balanced
        else:
            return 0.80  # Many features: linearize more for shorter equation
    ...
    def predict(self, X):
        ...
        r2_thresh = self._get_r2_threshold()
        pred = np.full(n, self.intercept_)

        for j, (thresholds, intervals) in self.shape_functions_.items():
            slope, offset, r2 = self.linear_approx_.get(j, (0, 0, 0))
            if r2 > r2_thresh and total_importance > 1e-10 and self.feature_importances_[j] / total_importance >= 0.01:
...
model_shorthand_name = "SmoothGAM_dynR2"
model_description = "SmoothAdditiveGAM with dynamic R2 threshold (0.60-0.80 based on #features)"
