...
class EnsembleGAMRegressor(BaseEstimator, RegressorMixin):
    """
    Ensemble of 3 SmartAdditiveGAMs with different random seeds.

    Each base model is a greedy additive boosted stumps model. The ensemble
    averages their predictions, reducing variance and improving rank without
    changing the display format (uses the first model's display).

    The ensemble prediction is smoother, which helps generalization on
    real datasets while the display from model 1 maintains interpretability.
    """
    ...
    def _fit_one(self, X, y, seed=42):
        """Fit one greedy additive stumps model."""
        ...
        for round_idx in range(self.n_rounds):
            ...
            # Slight randomization: shuffle feature evaluation order
        ...
        return intercept, shape_functions

    def _predict_one(self, X, intercept, shape_functions):
        n = X.shape[0]
        pred = np.full(n, intercept)
        for j, (thresholds, intervals) in shape_functions.items():
            xj = X[:, j]
    ...
    def fit(self, X, y):
        ...
        for k in range(self.n_estimators):
            intercept, shape_functions = self._fit_one(X, y, seed=42 + k * 7)
            self.models_.append((intercept, shape_functions))

        # For display, use model 0's shape functions and compute linear approx
        self.intercept_ = self.models_[0][0]
        self.shape_functions_ = self.models_[0][1]
    ...
    def predict(self, X):
        ...
        for intercept, shape_functions in self.models_:
            preds += self._predict_one(X, intercept, shape_functions)
...
EnsembleGAMRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "EnsembleGAM_v1"
model_description = "Ensemble of 3 SmartAdditiveGAMs with different seeds, averaged predictions"
model_defs = [(model_shorthand_name, EnsembleGAMRegressor())]
