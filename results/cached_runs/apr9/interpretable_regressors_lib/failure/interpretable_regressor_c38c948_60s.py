...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if resid_frac > 0.20:  # Very conservative: only when >20% variance remains
            self.tiny_gbm_ = _GBR(n_estimators=60, max_depth=1, learning_rate=0.05,
                                   random_state=42)
...
model_shorthand_name = "SmoothGAM_60s"
model_description = "SmoothAdditiveGAM + 60 stumps GBM when >20% var"
