...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # For features classified as linear (R²>0.70), recalibrate their
        # slopes using Ridge on the linear features only
        linear_js = [j for j in self.linear_approx_
                    if self.linear_approx_[j][2] > 0.70
                    and sum(self.feature_importances_) > 1e-10
                    and self.feature_importances_[j] / sum(self.feature_importances_) >= 0.01]
        if len(linear_js) >= 1:
            X_lin = X[:, linear_js]
            # Compute residual after nonlinear features
            nonlinear_pred = np.full(n_samples, 0.0)
            ...
            y_for_linear = y - self.intercept_ - nonlinear_pred

            ridge_recal = RidgeCV(cv=3)
            ridge_recal.fit(X_lin, y_for_linear)
            # Update linear approximations with recalibrated coefficients
            for idx, j in enumerate(linear_js):
                slope = float(ridge_recal.coef_[idx])
                offset = float(ridge_recal.intercept_ / len(linear_js)) if idx == 0 else 0.0
                self.linear_approx_[j] = (slope, offset, self.linear_approx_[j][2])
...
model_shorthand_name = "SmoothGAM_linRecal"
model_description = "SmoothAdditiveGAM + Ridge recalibration of linear feature slopes only"
