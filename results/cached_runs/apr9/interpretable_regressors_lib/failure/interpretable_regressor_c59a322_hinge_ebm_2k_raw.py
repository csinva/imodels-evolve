...
class HingeEBMRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # Stage 2: EBM on residuals (only if significant variance remains)
    ...
    def __str__(self):
        ...
        lines = [
            f"Sparse Linear Regression (Lasso, α={self.lasso_.alpha_:.4g}):",
        ]
        ...
        lines.append("")
        lines.append(f"Active terms ({len(active)} non-zero):")
        for n, c in active:
            lines.append(f"  {n}: {c:.4f}")

        zeroed = sum(1 for c in coefs if abs(c) <= 1e-6)
        if zeroed > 0:
            lines.append(f"  Features with zero coefficients (excluded): {zeroed} features")
        lines.append(f"  intercept: {intercept:.4f}")

        lines.append("")
        lines.append("Note: max(0, expr) = expr if expr > 0, else 0")
...
model_shorthand_name = "HingeEBM_2k_raw"
model_description = "HingeEBM 2 knots with raw hinge display + conditional EBM (>10% residual var)"
