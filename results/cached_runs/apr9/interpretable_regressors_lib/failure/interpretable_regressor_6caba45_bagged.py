...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def _fit_stumps(self, X, y, n_samples, n_features, seed_offset=0):
        """Fit greedy additive stumps on (X, y). Returns feature_stumps dict."""
        intercept = float(np.mean(y))
        feature_stumps = defaultdict(list)
        residuals = y - intercept

        # Add slight randomization by shuffling feature evaluation order
        rng = np.random.RandomState(42 + seed_offset)

        for _ in range(self.n_rounds):
            best_stump = None
            best_reduction = -np.inf
            feat_order = rng.permutation(n_features) if seed_offset > 0 else range(n_features)
            for j in feat_order:
                xj = X[:, j]
                order = np.argsort(xj)
                xj_sorted = xj[order]
        ...
        return intercept, feature_stumps

    def _collapse_stumps(self, feature_stumps, n_features):
        """Collapse stumps into shape functions with Laplacian smoothing."""
        shape_functions = {}
        for j in range(n_features):
            ...
            if len(intervals) > 2:
                ...
                for _ in range(3):
                    ...
                    smooth = new_s
                intervals = smooth
            shape_functions[j] = (thresholds, intervals)
    ...
    def fit(self, X, y):
        ...
        # Fit 3 models with different random seeds and average shape functions
        all_intercepts = []
        all_shapes = []
        for seed in range(3):
            intercept, stumps = self._fit_stumps(X, y, n_samples, n_features, seed_offset=seed)
            shapes = self._collapse_stumps(stumps, n_features)
            all_intercepts.append(intercept)
            all_shapes.append(shapes)

        self.intercept_ = float(np.mean(all_intercepts))

        # Average shape functions across the 3 models
        # Use the first model's thresholds as the common grid
        self.shape_functions_ = {}
        base_shapes = all_shapes[0]

        for j in range(n_features):
            if j not in base_shapes:
                continue
            thresholds, intervals = base_shapes[j]

            # Average the shape function values at each interval
            avg_intervals = list(intervals)
            n_models_with_j = 1

            for other_shapes in all_shapes[1:]:
                if j not in other_shapes:
                    continue
                other_thresholds, other_intervals = other_shapes[j]
                # Evaluate other model's shape at base model's midpoints
                for i in range(len(avg_intervals)):
                    if i == 0:
                        test_x = thresholds[0] - 1 if thresholds else 0
                    elif i >= len(thresholds):
                        test_x = thresholds[-1] + 1 if thresholds else 0
                    else:
                        test_x = (thresholds[i-1] + thresholds[i]) / 2
                    # Find the value in the other model
                    bin_idx = np.digitize(test_x, other_thresholds)
                    bin_idx = min(bin_idx, len(other_intervals) - 1)
                    avg_intervals[i] += other_intervals[bin_idx]
                n_models_with_j += 1

            # Average
            avg_intervals = [v / n_models_with_j for v in avg_intervals]
            self.shape_functions_[j] = (thresholds, avg_intervals)
...
model_shorthand_name = "BaggedGAM_v1"
model_description = "SmoothAdditiveGAM with 3-model bagging of shape functions"
