...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, n_rounds=200, learning_rate=0.1, min_samples_leaf=5,
                 linearity_bonus=0.05):
        self.n_rounds = n_rounds
        self.learning_rate = learning_rate
        self.min_samples_leaf = min_samples_leaf
        self.linearity_bonus = linearity_bonus
    ...
    def fit(self, X, y):
        ...
        for round_idx in range(self.n_rounds):
            ...
            for j in range(n_features):
                ...
                # Apply linearity bonus: slightly prefer features that already
                # have many stumps (concentrating splits on few features keeps
                # other features linear, improving R² → display consistency)
                n_existing_stumps = len(feature_stumps.get(j, []))
                bonus = 1.0 + self.linearity_bonus * min(n_existing_stumps, 10)
                adjusted_reduction = reduction[best_idx] * bonus

                if adjusted_reduction > best_reduction:
                    best_reduction = adjusted_reduction
...
model_shorthand_name = "SmoothGAM_linBonus"
model_description = "SmoothAdditiveGAM + linearity bonus (prefer splitting features that already have stumps)"
