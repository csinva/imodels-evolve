...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # Post-hoc Ridge recalibration on shape function features
        # This adjusts relative weights of features for better RMSE
        active_js = [j for j in self.shape_functions_ if self.feature_importances_[j] > 0]
        if len(active_js) >= 2:
            shape_features = np.zeros((n_samples, len(active_js)))
            for col, j in enumerate(active_js):
                thresholds, intervals = self.shape_functions_[j]
                bins = np.digitize(X[:, j], thresholds)
                shape_features[:, col] = np.array([intervals[b] for b in bins])

            recal = RidgeCV(cv=3, fit_intercept=True)
            recal.fit(shape_features, y)

            # Update shape functions with recalibration weights
            self.intercept_ = float(recal.intercept_)
            for col, j in enumerate(active_js):
                thresholds, intervals = self.shape_functions_[j]
                w = recal.coef_[col]
                self.shape_functions_[j] = (thresholds, [v * w for v in intervals])
                self.feature_importances_[j] *= abs(w)

            # Recompute linear approximations
            self.linear_approx_ = {}
            for j, (thresholds, intervals) in self.shape_functions_.items():
                xj = X[:, j]
                bins = np.digitize(xj, thresholds)
...
model_shorthand_name = "SmoothGAM_recal"
model_description = "SmoothAdditiveGAM + Ridge recalibration of shape function weights"
