...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # GBM on residuals (like HingeGBM) — only if significant variance remains
        ...
        for j_p, (thresholds, intervals) in self.shape_functions_.items():
            ...
            if r2 > 0.70 and total_imp > 1e-10 and self.feature_importances_[j_p] / total_imp >= 0.01:
                additive_pred += slope * X[:, j_p] + offset
            else:
                xj = X[:, j_p]
                bins = np.digitize(xj, thresholds)
                additive_pred += np.array([intervals[b] for b in bins])

        resid = y - additive_pred
        resid_frac = np.var(resid) / np.var(y) if np.var(y) > 1e-10 else 0
        if resid_frac > 0.15:  # Higher threshold than HingeGBM to preserve interp
            self.gbm_resid_ = _GBR(
                n_estimators=50, max_depth=2, learning_rate=0.05,
                subsample=0.8, random_state=42)
            self.gbm_resid_.fit(X, resid)
        else:
            self.gbm_resid_ = None
    ...
    def predict(self, X):
        ...
        # Add GBM residual correction if available
        if hasattr(self, 'gbm_resid_') and self.gbm_resid_ is not None:
            pred += self.gbm_resid_.predict(X)
...
model_shorthand_name = "SmoothGAM_GBMres"
model_description = "EXPERIMENT 100: SmoothAdditiveGAM + small GBM on residuals (50est depth=2 when >15% var)"
