...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        for j in range(n_features):
            ...
            # Enforce monotonicity if shape is nearly monotonic
            if len(intervals) >= 3:
                ...
                if n_inc >= 0.8 * len(diffs):  # Nearly increasing
                    for k in range(1, len(intervals)):
                        intervals[k] = max(intervals[k], intervals[k-1])
                elif n_dec >= 0.8 * len(diffs):  # Nearly decreasing
                    for k in range(1, len(intervals)):
                        intervals[k] = min(intervals[k], intervals[k-1])
...
model_shorthand_name = "SmoothGAM_mono"
model_description = "SmoothAdditiveGAM + monotonicity enforcement for nearly-monotonic features"
