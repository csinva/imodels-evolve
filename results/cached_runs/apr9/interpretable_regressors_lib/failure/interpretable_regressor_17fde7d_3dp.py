...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def __str__(self):
        ...
        if total_importance < 1e-10:
            return f"Constant model: y = {self.intercept_:.3f}"
        ...
        for j in sorted(linear_features.keys()):
            ...
            terms.append(f"{slope:.3f}*{name}")

        eq = " + ".join(terms) + f" + {combined_intercept:.3f}" if terms else f"{combined_intercept:.3f}"
        lines.append(f"  y = {eq}")
        lines.append("")
        ...
        for j, (slope, _) in sorted_linear:
            lines.append(f"  {feature_names[j]}: {slope:.3f}")
        lines.append(f"  intercept: {combined_intercept:.3f}")
        ...
        if nonlinear_features:
            ...
            for j in importance_sorted:
                ...
                for i, val in enumerate(intervals):
                    if i == 0:
                        parts.append(f"    {name} <= {thresholds[0]:.3f}: {val:+.4f}")
                    elif i == len(thresholds):
                        parts.append(f"    {name} >  {thresholds[-1]:.3f}: {val:+.4f}")
                    else:
                        parts.append(f"    {thresholds[i-1]:.3f} < {name} <= {thresholds[i]:.3f}: {val:+.4f}")
...
model_shorthand_name = "SmoothGAM_3dp"
model_description = "SmoothAdditiveGAM with 3 decimal places in display (simpler arithmetic)"
...
if __name__ == "__main__":
    ...
    upsert_overall_results([{
        ...
        "frac_interpretability_tests_passed": f"{n_passed / total:.3f}" if total > 0 else "nan",
        "status":                             "",
