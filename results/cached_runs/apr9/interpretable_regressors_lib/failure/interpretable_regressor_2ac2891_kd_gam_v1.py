...
class KnowledgeDistilledGAM(BaseEstimator, RegressorMixin):
    """
    Knowledge-distilled GAM: trains a greedy additive stumps model where
    50% of the target is the true y and 50% is a GBM's prediction.
    This guides the additive model to learn from the GBM's structure
    while still fitting the true targets.

    Uses the same display as SmoothAdditiveGAM (adaptive Ridge/piecewise).
    """
    ...
    def fit(self, X, y):
        ...
        # Train a GBM teacher
        teacher = _GBR(n_estimators=100, max_depth=3, random_state=42, subsample=0.8)
        teacher.fit(X, y)
        y_teacher = teacher.predict(X)

        # Blend targets: (1-w)*y + w*y_teacher
        y_blend = (1 - self.teacher_weight) * y + self.teacher_weight * y_teacher

        self.intercept_ = float(np.mean(y_blend))
        feature_stumps = defaultdict(list)
        residuals = y_blend - self.intercept_
        ...
        # Collapse + smooth + linear approx (same as SmoothAdditiveGAM)
            ...
            if len(intervals) > 2:
                ...
                for _ in range(3):
                    ...
                    for k in range(1, len(smooth) - 1):
                        new.append(0.6 * smooth[k] + 0.2 * smooth[k-1] + 0.2 * smooth[k+1])
                    new.append(smooth[-1])
                    smooth = new
                intervals = smooth
...
KnowledgeDistilledGAM.__module__ = "interpretable_regressor"

model_shorthand_name = "KD_GAM_v1"
model_description = "Knowledge-distilled GAM: additive stumps trained on 70% true y + 30% GBM predictions"
model_defs = [(model_shorthand_name, KnowledgeDistilledGAM())]
