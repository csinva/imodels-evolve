...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X):
        ...
        # Separate linear vs nonlinear features (same logic as __str__)
        linear_intercept = self.intercept_
        pred = np.zeros(n)
        ...
        for j, (thresholds, intervals) in self.shape_functions_.items():
            ...
            if r2 > 0.90:
                # Use linear approximation for consistent display/predict
                pred += slope * X[:, j] + offset
            else:
                # Use full piecewise for nonlinear features
                xj = X[:, j]
                bins = np.digitize(xj, thresholds)
                pred += np.array([intervals[b] for b in bins])

        # Add features below importance threshold using piecewise
        for j, (thresholds, intervals) in self.shape_functions_.items():
            if total_importance > 1e-10 and self.feature_importances_[j] / total_importance >= 0.01:
                continue
            xj = X[:, j]
    ...
    def __str__(self):
        ...
        # Show nonlinear features as piecewise corrections (simplified for readability)
        if nonlinear_features:
            ...
            for j in importance_sorted:
                ...
                # Simplify to at most 5 display bins by merging adjacent similar intervals
                disp_thresh = list(thresholds)
                disp_intervals = list(intervals)
                while len(disp_thresh) > 5:
                    # Find pair with smallest difference
                    min_diff = float('inf')
                    min_idx = 0
                    for k in range(len(disp_intervals) - 1):
                        diff = abs(disp_intervals[k] - disp_intervals[k + 1])
                        if diff < min_diff:
                            min_diff = diff
                            min_idx = k
                    merged = (disp_intervals[min_idx] + disp_intervals[min_idx + 1]) / 2
                    disp_intervals[min_idx] = merged
                    disp_intervals.pop(min_idx + 1)
                    disp_thresh.pop(min_idx)

                parts = []
                for i, val in enumerate(disp_intervals):
                    if i == 0:
                        parts.append(f"    {name} <= {disp_thresh[0]:.4f}: {val:+.4f}")
                    elif i == len(disp_thresh):
                        parts.append(f"    {name} >  {disp_thresh[-1]:.4f}: {val:+.4f}")
                    else:
                        parts.append(f"    {disp_thresh[i-1]:.4f} < {name} <= {disp_thresh[i]:.4f}: {val:+.4f}")
...
model_shorthand_name = "SmartAddGAM_compact"
model_description = "SmartAdditiveGAM R2=0.90 with nonlinear features simplified to 5 display bins"
