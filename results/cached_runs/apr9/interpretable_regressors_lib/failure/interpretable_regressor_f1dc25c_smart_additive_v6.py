...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def __str__(self):
        ...
        # Collect ALL active features' linear slopes and combined intercept
        active_features = {}  # j -> slope
        ...
        # Build Ridge-style display showing ALL features as linear coefficients
        lines = [
        ...
        for j in sorted(active_features.keys()):
            ...
            terms.append(f"{active_features[j]:.4f}*{name}")
        ...
        sorted_feats = sorted(active_features.items(), key=lambda x: abs(x[1]), reverse=True)
        for j, slope in sorted_feats:
        ...
        inactive = [feature_names[j] for j in range(self.n_features_in_) if j not in active_features]
        if inactive:
...
model_shorthand_name = "SmartAdditive_v6"
model_description = "SmartAdditive with all-linear display (all features shown as Ridge coefficients)"
