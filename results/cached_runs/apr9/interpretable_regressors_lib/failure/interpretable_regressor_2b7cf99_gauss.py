...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        for j in range(n_features):
            ...
            # Gaussian-style smoothing: 5-point stencil (wider, smoother)
            if len(intervals) > 4:
                smooth_intervals = list(intervals)
                for _ in range(2):  # 2 passes of wider smoothing
                    new_intervals = [smooth_intervals[0], smooth_intervals[1]]
                    for k in range(2, len(smooth_intervals) - 2):
                        new_intervals.append(
                            0.1 * smooth_intervals[k-2] +
                            0.2 * smooth_intervals[k-1] +
                            0.4 * smooth_intervals[k] +
                            0.2 * smooth_intervals[k+1] +
                            0.1 * smooth_intervals[k+2]
                        )
                    new_intervals.extend(smooth_intervals[-2:])
                    smooth_intervals = new_intervals
                intervals = smooth_intervals
            elif len(intervals) > 2:
                # Fall back to 3-point for short intervals
                smooth_intervals = list(intervals)
                for _ in range(3):
                    new_s = [smooth_intervals[0]]
                    for k in range(1, len(smooth_intervals) - 1):
                        new_s.append(0.6 * smooth_intervals[k] + 0.2 * smooth_intervals[k-1] + 0.2 * smooth_intervals[k+1])
                    new_s.append(smooth_intervals[-1])
                    smooth_intervals = new_s
                intervals = smooth_intervals
...
model_shorthand_name = "SmoothGAM_gauss"
model_description = "SmoothAdditiveGAM with 5-point Gaussian-style smoothing kernel"
