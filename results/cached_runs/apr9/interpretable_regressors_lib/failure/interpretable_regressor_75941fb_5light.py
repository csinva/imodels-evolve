    ...
    def fit(self, X, y):
        ...
        for j in range(n_features):
            ...
            if len(intervals) > 2:
                ...
                for _ in range(5):  # 5 passes of lighter smoothing
                    new_intervals = [smooth_intervals[0]]
                    for k in range(1, len(smooth_intervals) - 1):
                        new_intervals.append(
                            0.7 * smooth_intervals[k] +
                            0.15 * smooth_intervals[k - 1] +
                            0.15 * smooth_intervals[k + 1]
                        )
...
model_shorthand_name = "SmoothGAM_5light"
model_description = "SmoothAdditiveGAM with 5 passes of lighter smoothing (0.7/0.15/0.15)"
