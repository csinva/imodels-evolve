...
class ProceduralGAMRegressor(BaseEstimator, RegressorMixin):
    """
    Additive GAM with procedural display: instead of showing model parameters,
    shows a step-by-step prediction procedure the LLM can execute.

    Same SmoothAdditiveGAM backbone (greedy stumps + smoothing + hybrid predict),
    but __str__ is formatted as explicit computation instructions.

    Innovation: "how to use" instead of "what the model is".
    """
    ...
    def fit(self, X, y):
        ...
        # Collapse + smooth + linear approx (same pipeline)
    ...
    def __str__(self):
        ...
        # Procedural display: step-by-step prediction instructions
        lines = ["How to compute the prediction:"]
        lines.append("")
        ...
        lines.append(f"Step {step}: Compute y = {eq}")
        step += 1

        # Step 2+: Nonlinear corrections
        for j in sorted(nonlinear_features.keys(), key=lambda j: self.feature_importances_[j], reverse=True):
            ...
            lines.append(f"")
            lines.append(f"Step {step}: Add correction for {name}:")
            for i, val in enumerate(intervals):
                if i == 0:
                    lines.append(f"  if {name} <= {thresholds[0]:.4f}: add {val:+.4f}")
                elif i == len(thresholds):
                    lines.append(f"  if {name} >  {thresholds[-1]:.4f}: add {val:+.4f}")
                else:
                    lines.append(f"  if {thresholds[i-1]:.4f} < {name} <= {thresholds[i]:.4f}: add {val:+.4f}")
            step += 1
        ...
        if inactive:
            lines.append("")
            lines.append(f"Note: {', '.join(inactive)} have no effect (zero contribution)")
...
ProceduralGAMRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "ProceduralGAM_v1"
model_description = "SmoothAdditiveGAM with procedural step-by-step display format"
model_defs = [(model_shorthand_name, ProceduralGAMRegressor())]
