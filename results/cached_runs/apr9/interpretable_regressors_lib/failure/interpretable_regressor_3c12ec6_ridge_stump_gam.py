...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # First fit Ridge to capture linear effects
        self.ridge_ = RidgeCV(cv=3)
        self.ridge_.fit(X, y)
        y_ridge = self.ridge_.predict(X)
        y_residual = y - y_ridge

        # The additive stumps will model the residuals (nonlinear effects)
        self.intercept_ = float(np.mean(y_residual))
        ...
        if n_samples >= 50:
            ...
            train_mask = ~val_mask
            X_tr = X[train_mask]
            y_tr = y_residual[train_mask]  # Use residuals for stumps
            X_val, y_val = X[val_mask], y[val_mask]  # Original y for validation
            n_train = X_tr.shape[0]
        else:
            X_tr = X
            y_tr = y_residual  # Use residuals
        ...
        if X_val is not None and n_rounds_used > 0:
            ...
            feature_stumps = defaultdict(list)
            residuals_full = y_residual - self.intercept_
    ...
    def _predict_from_stumps(self, X, feature_stumps):
        ...
        n = X.shape[0]
        pred = self.ridge_.predict(X) + self.intercept_
    ...
    def predict(self, X):
        ...
        # Start with Ridge prediction
        pred = self.ridge_.predict(X)

        # Add stump corrections for residuals
        pred += self.intercept_
        total_importance = sum(self.feature_importances_)

        for j, (thresholds, intervals) in self.shape_functions_.items():
            # Use piecewise for all stump corrections
            xj = X[:, j]
            bins = np.digitize(xj, thresholds)
            pred += np.array([intervals[b] for b in bins])
    ...
    def __str__(self):
        ...
        # Show the Ridge equation (which captures linear effects perfectly)
        ...
        # Show nonlinear stump corrections if they have significant effect
        total_importance = sum(self.feature_importances_)
        if total_importance > 0.5:  # Only show if stump corrections are meaningful
            nonlinear_features = {}
            for j in self.shape_functions_:
                if self.feature_importances_[j] / total_importance >= 0.05:
                    nonlinear_features[j] = self.shape_functions_[j]

            if nonlinear_features:
                lines.append("")
                lines.append("Nonlinear corrections (add to the linear prediction above):")
        ...
        # Zero features
        inactive = [feature_names[j] for j in range(self.n_features_in_)
                    if abs(coefs[j]) < 1e-6 and j not in self.shape_functions_]
        if inactive:
            lines.append("")
...
model_shorthand_name = "RidgeStumpGAM"
model_description = "Ridge backbone + smoothed additive stumps on residuals with adaptive display"
