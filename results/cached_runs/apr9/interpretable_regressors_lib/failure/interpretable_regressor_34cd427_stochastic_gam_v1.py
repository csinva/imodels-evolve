...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, n_rounds=300, learning_rate=0.1, min_samples_leaf=5,
                 subsample=0.8, col_subsample=0.8):
    ...
    def fit(self, X, y):
        ...
        for round_idx in range(self.n_rounds):
            ...
            row_idx = rng.choice(n_samples, n_sub, replace=False)
            X_sub = X[row_idx]
            r_sub = residuals[row_idx]

            # Column subsampling
            n_col = max(1, int(n_features * self.col_subsample))
            col_idx = rng.choice(n_features, n_col, replace=False)
            ...
            # Update residuals on ALL samples (not just subsample)
            mask = X[:, j] <= threshold
...
model_shorthand_name = "StochasticGAM_v1"
model_description = "Stochastic additive stumps (300 rounds, subsample=0.8, col_subsample=0.8) with adaptive display R2=0.90"
