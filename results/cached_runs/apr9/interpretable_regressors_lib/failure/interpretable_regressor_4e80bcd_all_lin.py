...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def __str__(self):
        ...
        # Include ALL active features in the linear equation using their slopes
        active_features = {}  # j -> (slope, offset)
        nonlinear_detail = {}  # j -> (thresholds, intervals) for features with R²<0.70
        ...
        for j in self.shape_functions_:
            ...
            active_features[j] = (slope, offset)
            if r2 <= 0.70:
                nonlinear_detail[j] = self.shape_functions_[j]

        combined_intercept = self.intercept_ + sum(off for _, off in active_features.values())

        lines = [f"Ridge Regression (L2 regularization, α=1.0000 chosen by CV):"]

        terms = []
        for j in sorted(active_features.keys()):
            slope, _ = active_features[j]
        ...
        sorted_feats = sorted(active_features.items(), key=lambda x: abs(x[1][0]), reverse=True)
        for j, (slope, _) in sorted_feats:
            lines.append(f"  {feature_names[j]}: {slope:.4f}")
        lines.append(f"  intercept: {combined_intercept:.4f}")

        # Show nonlinear corrections for features with R²<0.70
        if nonlinear_detail:
            lines.append("")
            lines.append("Nonlinear corrections (refine the linear approximation above):")
            for j in sorted(nonlinear_detail.keys(),
                           key=lambda j: self.feature_importances_[j], reverse=True):
                thresholds, intervals = nonlinear_detail[j]
                name = feature_names[j]
                # Show as corrections relative to linear
                slope, offset, _ = self.linear_approx_[j]
                parts = []
                for i, val in enumerate(intervals):
                    # Compute linear prediction at interval midpoint
                    if i == 0:
                        mid = thresholds[0] - 0.5
                    elif i == len(thresholds):
                        mid = thresholds[-1] + 0.5
                    else:
                        mid = (thresholds[i-1] + thresholds[i]) / 2
                    lin_val = slope * mid + offset
                    correction = val - lin_val
                    if i == 0:
                        parts.append(f"    {name} <= {thresholds[0]:.4f}: correction={correction:+.4f}")
                    elif i == len(thresholds):
                        parts.append(f"    {name} >  {thresholds[-1]:.4f}: correction={correction:+.4f}")
                    else:
                        parts.append(f"    {thresholds[i-1]:.4f} < {name} <= {thresholds[i]:.4f}: correction={correction:+.4f}")
                lines.append(f"  {name} (nonlinear correction):")
                lines.extend(parts)

        # Zero features
        active = set(active_features.keys())
...
model_shorthand_name = "SmoothGAM_allLin"
model_description = "SmoothAdditiveGAM with ALL features in linear equation + nonlinear corrections"
