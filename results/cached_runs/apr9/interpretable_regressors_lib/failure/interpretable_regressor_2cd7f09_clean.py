...
class SmartAdditiveRegressor(BaseEstimator, RegressorMixin):
    ...
    def __str__(self):
        ...
        # Show nonlinear features as piecewise corrections (cleaned up for readability)
        if nonlinear_features:
            ...
            for j in importance_sorted:
                ...
                # Clean up: merge adjacent intervals with very similar values
                clean_thresh = list(thresholds)
                clean_intervals = list(intervals)
                i = 0
                while i < len(clean_intervals) - 1:
                    if abs(clean_intervals[i] - clean_intervals[i + 1]) < 0.05 * self.feature_importances_[j]:
                        merged = (clean_intervals[i] + clean_intervals[i + 1]) / 2
                        clean_intervals[i] = merged
                        clean_intervals.pop(i + 1)
                        if i < len(clean_thresh):
                            clean_thresh.pop(i)
                    else:
                        i += 1

                parts = []
                for i, val in enumerate(clean_intervals):
                    if abs(val) < 0.01:
                        val = 0.0
                    if i == 0 and len(clean_thresh) > 0:
                        parts.append(f"    {name} <= {clean_thresh[0]:.4f}: {val:+.4f}")
                    elif i == len(clean_thresh):
                        parts.append(f"    {name} >  {clean_thresh[-1]:.4f}: {val:+.4f}")
                    elif i < len(clean_thresh):
                        parts.append(f"    {clean_thresh[i-1]:.4f} < {name} <= {clean_thresh[i]:.4f}: {val:+.4f}")
...
model_shorthand_name = "SmoothGAM_clean"
model_description = "SmoothAdditiveGAM with cleaned piecewise display (merge similar intervals)"
