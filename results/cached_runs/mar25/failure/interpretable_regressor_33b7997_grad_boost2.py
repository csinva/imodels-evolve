...
class InterpretableRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, min_samples_leaf=5, n_stages=2, learning_rate=0.5):
        self.min_samples_leaf = min_samples_leaf
        self.n_stages = n_stages
        self.learning_rate = learning_rate

    def _cv_best_leaves(self, X, y):
    ...
    def fit(self, X, y):
        ...
        self.stages_ = []  # list of (max_leaf_nodes, tree)
        for stage in range(self.n_stages):
            best_nodes = self._cv_best_leaves(X, residual)
            tree = DecisionTreeRegressor(
            ...
            tree.fit(X, residual)
            self.stages_.append((best_nodes, tree))
            residual = residual - self.learning_rate * tree.predict(X)
    ...
    def __str__(self):
        ...
        lines = [
            f"Gradient Boosted Tree Model ({len(self.stages_)} stages, "
            f"learning_rate={self.learning_rate}):",
            f"prediction = {self.intercept_:.4f} + {self.learning_rate} * tree_1(X) "
            f"+ {self.learning_rate} * tree_2(X)",
            "",
        ]
        for i, (n_leaves, tree) in enumerate(self.stages_):
            tree_text = export_text(tree, feature_names=feature_names, max_depth=10)
            actual = tree.get_n_leaves()
            lines.append(f"--- Stage {i+1} tree ({actual} leaves) ---")
...
if __name__ == "__main__":
    ...
    upsert_overall_results([{
        ...
        "description":                        "2-stage gradient boosted CV-tuned trees with learning_rate=0.5",
    }], RESULTS_DIR)
