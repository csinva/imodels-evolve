...
class InterpretableRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, min_samples_leaf=5, max_features_select=None):
    ...
    def fit(self, X, y):
        ...
        # Feature selection: keep top features by absolute correlation with y
        max_feat = self.max_features_select or min(X.shape[1], 15)
        ...
        for n_leaves in options:
            ...
            for train_idx, val_idx in kf.split(X_sel):
                ...
                tree.fit(X_sel[train_idx], y[train_idx])
                scores.append(tree.score(X_sel[val_idx], y[val_idx]))
    ...
    def predict(self, X):
        ...
        X = np.asarray(X, dtype=np.float64)
        return self.tree_.predict(X[:, self.selected_features_])

    def __str__(self):
        check_is_fitted(self, "tree_")
        feature_names = [f"x{i}" for i in self.selected_features_]
...
if __name__ == "__main__":
    ...
    upsert_overall_results([{
        ...
        "description":                        "CV-selected tree on correlation-selected top features",
    }], RESULTS_DIR)
