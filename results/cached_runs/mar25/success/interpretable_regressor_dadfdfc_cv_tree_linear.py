...
class InterpretableRegressor(BaseEstimator, RegressorMixin):
    ...
    def __str__(self):
        ...
        lines = [
            ...
            tree_text,
            "",
            "Approximate linear feature effects (from OLS fit to same data):",
            f"  intercept: {self.linear_.intercept_:.4f}",
        ]
        for i, c in enumerate(self.linear_.coef_):
            lines.append(f"  x{i}: {c:+.4f} (per unit change in x{i}, output changes by ~{c:.4f})")
        return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    upsert_overall_results([{
        ...
        "description":                        "CV-tuned tree with linear feature effects summary in __str__",
    }], RESULTS_DIR)
