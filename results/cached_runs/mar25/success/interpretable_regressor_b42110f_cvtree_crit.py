...
class InterpretableRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        options = self.max_leaf_nodes_options or [4, 6, 8, 10, 12, 14, 16, 20, 25, 30]
        ...
        # Also try criterion='friedman_mse' with best nodes
        for criterion in ['squared_error', 'friedman_mse']:
            ...
            if mean_score > best_score:
                best_score = mean_score
                self.best_criterion_ = criterion

        if not hasattr(self, 'best_criterion_'):
            self.best_criterion_ = 'squared_error'

        self.tree_ = DecisionTreeRegressor(
            max_leaf_nodes=best_nodes,
            min_samples_leaf=self.min_samples_leaf,
            criterion=self.best_criterion_,
            random_state=42,
        )
        self.tree_.fit(X, y)
...
if __name__ == "__main__":
    ...
    upsert_overall_results([{
        ...
        "description":                        "CV-selected decision tree with criterion tuning: max_leaf_nodes + squared_error vs friedman_mse",
    }], RESULTS_DIR)
