...
class HingeLinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, n_knots=3, max_terms=15, tree_depth=2):
    ...
    def fit(self, X, y):
        ...
        # Stage 2: Fit shallow tree on residuals for nonlinear correction
        linear_pred = self.lasso_.predict(Z)
        residual = y - linear_pred
        residual_var = np.var(residual)
        original_var = np.var(y)

        # Only add tree if significant residual variance remains
        self.use_tree_ = residual_var > 0.1 * original_var
        if self.use_tree_:
    ...
    def predict(self, X):
        ...
        if self.use_tree_ and self.tree_ is not None:
    ...
    def __str__(self):
        ...
        if not active_terms:
            ...
            lines.append("")
            lines.append("HOW TO PREDICT: substitute feature values into the equation above.")

        # Show residual tree if used
        if self.use_tree_ and self.tree_ is not None:
            names = [f"x{i}" for i in range(self.n_features_)]
            tree_text = export_text(self.tree_, feature_names=names, max_depth=4)
            lines.append("")
            lines.append("RESIDUAL CORRECTION TREE (add this to the linear prediction):")
            lines.append(tree_text)
            lines.append("TOTAL PREDICTION = linear equation above + tree correction above")
...
if __name__ == "__main__":
    ...
    upsert_overall_results([{
        ...
        "status":                             "",
        "model_name":                         "HingeLinear",
        "description":                        "Sparse piecewise-linear model with hinge basis functions and LassoCV",
    }], RESULTS_DIR)
