...
class HingeLinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, n_knots=3, max_terms=15, tree_depth=3):
    ...
    def __str__(self):
        ...
        if self.use_tree_ and self.tree_ is not None:
            ...
            lines.append("")
            lines.append("RESIDUAL CORRECTION TREE (add this to the linear prediction above):")
            lines.append(tree_text)
            lines.append("TOTAL = linear equation result + tree leaf value")
        else:
...
if __name__ == "__main__":
    ...
    model_defs = [("HingeTreeHybrid", HingeLinearRegressor())]
    ...
    upsert_overall_results([{
        ...
        "status":                             "",
        "model_name":                         "HingeTreeHybrid",
        "description":                        "Hybrid: sparse hinge-linear + depth-3 residual tree correction",
    }], RESULTS_DIR)
