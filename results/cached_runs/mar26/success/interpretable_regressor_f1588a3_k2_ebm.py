...
class HingeLinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # Stage 2: Fit EBM on residuals (powerful additive model)
        ...
        if self.use_tree_:
            self.gbm_ = ExplainableBoostingRegressor(
                random_state=42,
                outer_bags=2,
                max_rounds=500,
            )
    ...
    def __str__(self):
        ...
        if self.use_tree_ and self.gbm_ is not None:
            lines.append("")
            lines.append("RESIDUAL CORRECTION (additive model for nonlinear adjustment)")
        else:
...
if __name__ == "__main__":
    ...
    model_defs = [("HingeK2EBM", HingeLinearRegressor())]
    ...
    upsert_overall_results([{
        ...
        "status":                             "",
        "model_name":                         "HingeK2EBM",
        "description":                        "2-knot hinge-linear + EBM residual correction",
    }], RESULTS_DIR)
