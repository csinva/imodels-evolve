...
class HingeLinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # Stage 2: Fit boosted shallow trees on residuals
        residual = y - self.lasso_.predict(Z)
        original_var = np.var(y)
        self.trees_ = []

        for t_idx in range(self.n_trees):
            residual_var = np.var(residual)
            if residual_var < 0.02 * original_var:
                break
            ...
            tree.fit(X, residual)
            residual = residual - self.lr * tree.predict(X)
            self.trees_.append(tree)

        self.use_tree_ = len(self.trees_) > 0
    ...
    def __str__(self):
        ...
        # Show residual trees if used
        if self.use_tree_ and len(self.trees_) > 0:
            ...
            for t_idx, tree in enumerate(self.trees_):
                ...
                lines.append("")
                lines.append(f"RESIDUAL CORRECTION TREE {t_idx+1} (multiply leaf value by {self.lr}):")
                lines.append(tree_text)
            lines.append("TOTAL = linear equation + sum of (tree leaf * learning_rate) for each tree")
        else:
...
if __name__ == "__main__":
    ...
    model_defs = [("HingeBoosted3", HingeLinearRegressor())]
    ...
    upsert_overall_results([{
        ...
        "status":                             "",
        "model_name":                         "HingeBoosted3",
        "description":                        "Boosted: sparse hinge-linear + 3 gradient-boosted depth-3 trees",
    }], RESULTS_DIR)
