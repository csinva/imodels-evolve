...
class HingeLinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, n_knots=3, max_terms=15, tree_depth=3, n_trees=3, lr=0.5):
    ...
    def fit(self, X, y):
        ...
        # For low-dimensional data, use OLS on hinge features (cleaner, more traceable)
        # For high-dimensional data, use LassoCV + GBM residuals
        self.use_ols_ = n_features <= 6
        ...
        if self.use_ols_:
            # OLS on hinge features — exact coefficients, perfect for LLM tracing
            self.lasso_ = LinearRegression()
            self.lasso_.fit(Z, y)
            self.coef_ = self.lasso_.coef_
            self.intercept_ = float(self.lasso_.intercept_)
            # Zero out very small coefficients for cleaner display
            self.coef_[np.abs(self.coef_) < 0.01] = 0
            self.active_mask_ = np.abs(self.coef_) > 1e-8
            self.n_active_ = int(self.active_mask_.sum())
            self.use_tree_ = False
            self.gbm_ = None
        else:
            # Stage 1: Fit sparse linear model
            ...
            # Stage 2: Fit GBM on residuals
            residual = y - self.lasso_.predict(Z)
            original_var = np.var(y)
            residual_var = np.var(residual)

            self.use_tree_ = residual_var > 0.02 * original_var
            if self.use_tree_:
    ...
    def predict(self, X):
        ...
        pred = self.lasso_.predict(Z)
        if self.use_tree_ and self.gbm_ is not None:
    ...
    def __str__(self):
        ...
        # Show GBM residual correction info
        if self.use_tree_ and self.gbm_ is not None:
            names = [f"x{i}" for i in range(self.n_features_)]
            lines.append("")
            lines.append("RESIDUAL CORRECTION (gradient boosted trees, adds nonlinear adjustment):")
            imp = self.gbm_.feature_importances_
            top_feats = np.argsort(imp)[::-1][:5]
            for idx in top_feats:
                if imp[idx] > 0.02:
                    lines.append(f"  {names[idx]}: importance={imp[idx]:.3f}")
            # Show first tree for traceability
            first_tree = self.gbm_.estimators_[0, 0]
            tree_text = export_text(first_tree, feature_names=names, max_depth=3)
            lines.append(f"\nFirst correction tree (lr=0.1, one of {self.gbm_.n_estimators_} trees):")
...
if __name__ == "__main__":
    ...
    model_defs = [("HingeAdaptive", HingeLinearRegressor())]
    ...
    upsert_overall_results([{
        ...
        "status":                             "",
        "model_name":                         "HingeAdaptive",
        "description":                        "Adaptive: OLS for small data, hinge+GBM for large data",
    }], RESULTS_DIR)
