...
class HingeLinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, n_knots=3, max_terms=15, max_features=10):
    ...
    def fit(self, X, y):
        ...
        # Stage 1: Feature prescreening using RidgeCV
        if n_features > self.max_features:
            ridge = RidgeCV(cv=min(3, n_samples))
            ridge.fit(X, y)
            importances = np.abs(ridge.coef_)
            self.selected_features_ = np.argsort(importances)[-self.max_features:]
        ...
        self.knots_ = {}
        for j_local in range(len(self.selected_features_)):
            quantiles = np.linspace(15, 85, self.n_knots)
            self.knots_[j_local] = np.percentile(X_sel[:, j_local], quantiles)

        # Build expanded feature matrix
        Z, self.term_names_ = self._build_features(X_sel)
    ...
    def _build_features(self, X_sel):
        """Build hinge basis expansion on selected features."""
        n_samples, n_local = X_sel.shape
        columns = []
        names = []

        for j_local in range(n_local):
            j_orig = self.selected_features_[j_local]
            xj = X_sel[:, j_local]
            fname = f"x{j_orig}"
    ...
    def predict(self, X):
        ...
        X = np.asarray(X, dtype=np.float64)
        X_sel = X[:, self.selected_features_]
        Z, _ = self._build_features(X_sel)
