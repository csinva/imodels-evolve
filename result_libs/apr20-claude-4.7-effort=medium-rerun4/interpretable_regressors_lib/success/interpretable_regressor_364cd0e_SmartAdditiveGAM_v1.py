...
class SmartAdditiveGAM(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X):
        ...
        for j, (thresholds, intervals) in self.shape_functions_.items():
            ...
            # Use linear form when the display would (for parity between __str__ and predict)
...
model_shorthand_name = "SmartAdditiveGAM_v1"
model_description = "Greedy boosted-stump GAM with Laplacian-smoothed shape functions; adaptive linear/piecewise display (R^2>0.70 → linear)"
model_defs = [(model_shorthand_name, SmartAdditiveGAM(n_rounds=200, learning_rate=0.1, min_samples_leaf=5, r2_linear_thresh=0.70))]
