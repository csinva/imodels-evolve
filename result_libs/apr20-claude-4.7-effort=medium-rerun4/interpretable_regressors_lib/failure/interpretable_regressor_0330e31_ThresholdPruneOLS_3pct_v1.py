...
class ThresholdPruneOLS(BaseEstimator, RegressorMixin):
    """Full OLS, then prune features with |std-coef| < threshold * max, refit OLS.
    Keeps OLS-level accuracy but removes clutter features for better interpretability."""

    def __init__(self, threshold_frac=0.03):
    ...
    def fit(self, X, y):
        ...
        # Fit OLS on standardized to get comparable std-coef magnitudes
        mu = X.mean(axis=0)
        sigma = X.std(axis=0)
        ...
        if not keep.any():
            keep = np.argsort(-std_coef)[:1]
            keep = np.isin(np.arange(self.n_features_in_), keep)
    ...
    def __str__(self):
        ...
        lines = [
            "Linear Regression (OLS after pruning features with tiny standardized coefficients):",
            f"  y = {self.intercept_:+.4f} {eqn}",
            "",
            "Active features (non-zero coefficients, units = per-unit change in feature):",
        ]
        ...
        if excluded:
            lines.append(f"  excluded (near-zero effect): {', '.join(excluded)}")
...
model_shorthand_name = "ThresholdPruneOLS_3pct_v1"
model_description = "OLS, prune features with standardized coef below 3% of max, OLS refit"
model_defs = [(model_shorthand_name, ThresholdPruneOLS(threshold_frac=0.03))]
