...
class InteractionGAM(BaseEstimator, RegressorMixin):
    """SmartAdditiveGAM + a single best pairwise interaction term (x_i * x_j).

    After fitting the additive GAM, search all (i,j) pairs for the bilinear
    product that most reduces residual SSE via a ridge fit; if R^2 > 0.02,
    add it to predict and display.
    """
    def __init__(self, n_rounds=200, learning_rate=0.1, min_samples_leaf=5,
                 r2_linear_thresh=0.70, min_r2_gain=0.02, top_k_features=10):
    ...
    def fit(self, X, y):
        ...
        self.gam_ = SmartAdditiveGAM(n_rounds=self.n_rounds, learning_rate=self.learning_rate,
                                     min_samples_leaf=self.min_samples_leaf,
                                     r2_linear_thresh=self.r2_linear_thresh, n_smooth_passes=3).fit(X, y)
        resid = y - self.gam_.predict(X)
        ss_tot = float(np.sum(resid ** 2))
        self.interaction_ = None  # (i, j, coef)
        if ss_tot > 1e-10:
            # Restrict search to top-K features by importance
            imp = self.gam_.feature_importances_
            K = min(self.top_k_features, p)
            top_feats = list(np.argsort(imp)[::-1][:K])
            best_gain = 0.0
            best_pair = None
            best_coef = 0.0
            for i_idx in range(len(top_feats)):
                for j_idx in range(i_idx, len(top_feats)):
                    i, j = top_feats[i_idx], top_feats[j_idx]
                    z = X[:, i] * X[:, j]
                    zz = float(np.dot(z, z))
                    if zz < 1e-10:
                        continue
                    coef = float(np.dot(z, resid) / zz)
                    pred_extra = coef * z
                    ss_res = float(np.sum((resid - pred_extra) ** 2))
                    gain = 1 - ss_res / ss_tot
                    if gain > best_gain:
                        best_gain = gain
                        best_pair = (i, j)
                        best_coef = coef
            if best_gain > self.min_r2_gain:
                self.interaction_ = (best_pair[0], best_pair[1], best_coef)
        return self

    def predict(self, X):
        check_is_fitted(self, "gam_")
        X = np.asarray(X, dtype=np.float64)
        pred = self.gam_.predict(X)
        if self.interaction_ is not None:
            i, j, c = self.interaction_
            pred = pred + c * X[:, i] * X[:, j]
        return pred
...
class StagewiseGAM(BaseEstimator, RegressorMixin):
    """Fit SmartAdditiveGAM, compute residuals, fit a second SmartAdditiveGAM
    on residuals. Merge the two GAMs into a single GAM (union thresholds,
    sum interval values) so predict == display.
    """
    def __init__(self, n_rounds_1=200, n_rounds_2=100, learning_rate=0.1,
                 min_samples_leaf=5, r2_linear_thresh=0.70):
    ...
    def fit(self, X, y):
        ...
        n, p = X.shape
        self.n_features_in_ = p
        g1 = SmartAdditiveGAM(n_rounds=self.n_rounds_1, learning_rate=self.learning_rate,
                              min_samples_leaf=self.min_samples_leaf,
                              r2_linear_thresh=self.r2_linear_thresh, n_smooth_passes=3).fit(X, y)
        resid = y - g1.predict(X)
        g2 = SmartAdditiveGAM(n_rounds=self.n_rounds_2, learning_rate=self.learning_rate,
                              min_samples_leaf=self.min_samples_leaf,
                              r2_linear_thresh=self.r2_linear_thresh, n_smooth_passes=3).fit(X, resid)
        # Merge: create a new SmartAdditiveGAM skeleton with combined shapes
        merged = SmartAdditiveGAM(n_rounds=1, learning_rate=self.learning_rate,
                                  min_samples_leaf=self.min_samples_leaf,
                                  r2_linear_thresh=self.r2_linear_thresh)
        merged.n_features_in_ = p
        merged.intercept_ = g1.intercept_ + g2.intercept_
        merged.shape_functions_ = {}
        merged.linear_approx_ = {}
        merged.feature_importances_ = np.zeros(p)
        for j in range(p):
            sf1 = g1.shape_functions_.get(j)
            sf2 = g2.shape_functions_.get(j)
            if sf1 is None and sf2 is None:
                continue
            # Gather all thresholds
            t1 = sf1[0] if sf1 else []
            t2 = sf2[0] if sf2 else []
            thresholds = sorted(set(list(t1) + list(t2)))
            intervals = []
            for i in range(len(thresholds) + 1):
                ...
                for sf in (sf1, sf2):
                    if sf is None:
                        continue
                    th, iv = sf
                    b = int(np.digitize([test_x], th)[0])
                    v += iv[b]
                intervals.append(v)
            merged.shape_functions_[j] = (thresholds, intervals)
            merged.feature_importances_[j] = max(intervals) - min(intervals)
            # Recompute linear approx
            xj = X[:, j]
            ...
            merged.linear_approx_[j] = (slope, offset, r2)
        self.gam_ = merged
        return self
...
StagewiseGAM.__module__ = "interpretable_regressor"
InteractionGAM.__module__ = "interpretable_regressor"

model_shorthand_name = "HybridGAM_v24"
model_description = "HybridGAM like v19 but r2_thresh=0.85 (more features displayed as linear → maybe cleaner)"
model_defs = [(model_shorthand_name, HybridGAM(gam_r2_thresh=0.85, n_residual_trees=300, residual_depth=10, residual_type="rf", residual_shrinkage=0.1))]
