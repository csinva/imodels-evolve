...
class OLSPlain(BaseEstimator, RegressorMixin):
    """Plain OLS on raw features. y = intercept + sum c_i * x_i."""
    ...
    def predict(self, X):
        ...
        return self.lr_.predict(np.asarray(X, dtype=float))
    ...
    def __str__(self):
        ...
        lines = [
            "OLS Linear Regression (plain, no regularization):",
            f"  y = {self.intercept_:+.4f} {equation}",
            "",
...
OLSPlain.__module__ = "interpretable_regressor"

InterpretableRegressor = OLSPlain
model_shorthand_name = "OLSPlain_v2"
model_description = "Plain OLS on raw features with clean readable formula"
model_defs = [(model_shorthand_name, OLSPlain())]
