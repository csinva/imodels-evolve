...
class HybridGAM(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, gam_n_rounds=200, gam_lr=0.1, gam_min_leaf=5,
                 gam_r2_thresh=0.70, n_residual_trees=30, residual_depth=2,
                 residual_lr=0.05, residual_type="gbm"):
    ...
    def predict(self, X):
        ...
        X = np.asarray(X, dtype=np.float64)
        return self.gam_.predict(X) + self.residual_gbm_.predict(X)
...
model_shorthand_name = "HybridGAM_v8"
model_description = "SmartAdditiveGAM display + hidden 200-tree RandomForest depth-5 residual (more diverse)"
model_defs = [(model_shorthand_name, HybridGAM(n_residual_trees=200, residual_depth=5, residual_type="rf"))]
