...
class SparseOLSTop5(BaseEstimator, RegressorMixin):
    """LassoCV-select top-K raw features, refit plain OLS. Produces a compact linear formula."""
    ...
    def fit(self, X, y):
        ...
        try:
            lasso = LassoCV(cv=min(5, max(2, len(y)//10)), random_state=0, max_iter=20000, n_alphas=30).fit(X, y)
            order = np.argsort(-np.abs(lasso.coef_))
        except Exception:
            order = np.arange(self.n_features_in_)
        selected = list(order[: self.max_features])
        if not selected:
            selected = list(range(min(self.max_features, self.n_features_in_)))
    ...
    def __str__(self):
        ...
        lines = [
            f"Sparse OLS (top-{self.max_features} features selected by LassoCV, then OLS refit):",
...
SparseOLSTop5.__module__ = "interpretable_regressor"

model_shorthand_name = "SparseOLSTop5_v2"
model_description = "LassoCV selects top-5 features, OLS refit, clean formula"
model_defs = [(model_shorthand_name, SparseOLSTop5(max_features=5))]
