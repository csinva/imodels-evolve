...
class LinearStepRuleRegressor(BaseEstimator, RegressorMixin):
    """Dense ridge backbone plus one optional threshold step correction."""

    def __init__(
        self,
        alpha_grid=(0.03, 0.1, 0.3, 1.0, 3.0, 10.0, 30.0),
        cv_folds=3,
        max_step_features=10,
        step_quantiles=(0.25, 0.5, 0.75),
    ...
    def _fit_augmented(self, X, y, alpha, feat_idx, threshold):
        step = (X[:, feat_idx] > threshold).astype(float).reshape(-1, 1)
        Xa = np.hstack([X, step])
        coef_aug, intercept = self._fit_ridge_raw(Xa, y, alpha)
        return coef_aug[:-1], float(coef_aug[-1]), intercept

    def _cv_augmented_mse(self, X, y, alpha, feat_idx, threshold, folds):
        errs = []
        for i in range(len(folds)):
            val_idx = folds[i]
            if val_idx.size == 0:
            ...
            coef_lin, coef_step, intercept = self._fit_augmented(Xt, yt, alpha, feat_idx, threshold)
            pred = intercept + Xv @ coef_lin + coef_step * (Xv[:, feat_idx] > threshold).astype(float)
    ...
    def fit(self, X, y):
        ...
        candidate_features = feat_order[: max(1, min(int(self.max_step_features), n_features))]

        folds = self._make_folds(n_samples)
        best_aug_mse = best_linear_cv_mse
        best_aug = None
        for feat_idx in candidate_features:
            for q in self.step_quantiles:
                threshold = float(np.quantile(X[:, feat_idx], q))
                mse = self._cv_augmented_mse(X, y, self.selected_alpha_, int(feat_idx), threshold, folds)
                if mse < best_aug_mse:
                    best_aug_mse = mse
                    best_aug = (int(feat_idx), threshold)
        ...
        if best_aug is not None and rel_gain >= float(self.min_rel_improvement):
            feat_idx, threshold = best_aug
            coef_full, step_coef, intercept = self._fit_augmented(X, y, self.selected_alpha_, feat_idx, threshold)
            self.step_feature_ = int(feat_idx)
            self.step_threshold_ = float(threshold)
            self.step_coef_ = float(step_coef)
            self.used_step_ = True
        else:
            self.step_feature_ = -1
            self.step_threshold_ = 0.0
            self.step_coef_ = 0.0
            self.used_step_ = False

        coef_full[np.abs(coef_full) < self.tiny_coef] = 0.0
        importances = np.abs(coef_full)
        if self.used_step_:
            importances[self.step_feature_] += abs(self.step_coef_)
    ...
    def predict(self, X):
        check_is_fitted(self, ["intercept_", "coef_", "used_step_"])
        X = np.asarray(X, dtype=float)
        pred = self.intercept_ + X @ self.coef_
        if self.used_step_:
            pred = pred + self.step_coef_ * (X[:, self.step_feature_] > self.step_threshold_).astype(float)
        return pred
    ...
    def __str__(self):
        ...
        for j in range(len(self.coef_)):
            eq_terms.append(f"{float(self.coef_[j]):+.6f}*x{j}")
        if self.used_step_ and abs(self.step_coef_) > self.tiny_coef:
            eq_terms.append(
                f"{float(self.step_coef_):+.6f}*I(x{self.step_feature_}>{float(self.step_threshold_):.6f})"
            )
        ...
        for j in order[: min(10, len(order))]:
            top_lines.append(
                f"x{int(j)}: coef={float(self.coef_[j]):+.4f}, importance={float(self.feature_importances_[j]):.3f}"
            )

        lines = ["Prediction equation:", "y = " + " ".join(eq_terms)]
        if self.used_step_:
            lines.append("Indicator rule: I(condition)=1 if condition is true, else 0.")
        lines.extend(["", "Most influential linear features:"] + top_lines)
        return "\n".join(lines)
...
LinearStepRuleRegressor.__module__ = "interpretable_regressor"

model_shorthand_name = "LinearStepRule_v1"
model_description = "Custom CV ridge backbone over all features with one validation-selected binary threshold step correction and calculator-first equation output"
model_defs = [(model_shorthand_name, LinearStepRuleRegressor())]
