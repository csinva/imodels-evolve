...
class TransparentDistilledForestBoostRegressor(BaseEstimator, RegressorMixin):
    """
    Dual-path regressor:
    - Batch path (> symbolic_n_rows): calibrated ensemble for predictive performance.
    - Single-row path (<= symbolic_n_rows): compact symbolic equation for simulatability.
    """

    def __init__(
        self,
        teacher_gbm_estimators=180,
        teacher_gbm_lr=0.05,
        teacher_gbm_depth=3,
        teacher_rf_estimators=180,
        teacher_rf_max_depth=14,
        teacher_et_estimators=180,
        teacher_et_max_depth=None,
        teacher_min_samples_leaf=2,
        teacher_ridge_alpha=1.0,
        val_fraction=0.2,
        student_max_linear_terms=6,
        student_alpha=1e-6,
        student_teacher_mix=0.25,
        hinge_quantiles=(0.2, 0.4, 0.6, 0.8),
        hinge_top_features=2,
        min_hinge_gain=0.01,
        coef_decimals=2,
        coef_tol=1e-8,
        symbolic_n_rows=1,
    ...
    def _fit_teacher(self, X, y):
        ...
        self.teacher_rf_ = RandomForestRegressor(
            ...
            max_depth=None if self.teacher_rf_max_depth is None else int(self.teacher_rf_max_depth),
            min_samples_leaf=int(self.teacher_min_samples_leaf),
            max_features=0.7,
            random_state=int(self.random_state),
            n_jobs=1,
        ...
        if float(np.sum(blend_w)) <= 1e-12:
            blend_w = np.array([1.0, 0.0, 0.0, 0.0], dtype=float)
            blend_inter = 0.0
        else:
    ...
    def _predict_teacher(self, X):
        ...
        return self.teacher_blend_intercept_ + (
            ...
            + w[2] * self.teacher_et_.predict(X)
            + w[3] * self.teacher_ridge_.predict(X)
        )
    ...
    def _eval_term(X, term):
        ...
        if term["type"] == "hinge":
            ...
            return TransparentDistilledForestBoostRegressor._hinge_col(
                X[:, feat], float(term["knot"]), int(term["direction"])
            )
    ...
    def __str__(self):
        ...
        lines = [
            "Transparent Distilled Forest-Boost Regressor",
            ...
            "  - For max(0, a), clip negative values to 0 before multiplying by its coefficient.",
...
TransparentDistilledForestBoostRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "TransparentDistilledForestBoost_v1"
model_description = "Holdout-calibrated GBM/RF/ExtraTrees/Ridge batch ensemble with sparse distilled linear-plus-hinge symbolic single-row equation"
model_defs = [(model_shorthand_name, TransparentDistilledForestBoostRegressor())]
