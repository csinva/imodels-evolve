...
class AdaptiveStackedTeacherAuditAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Dual-path architecture with a regime-adaptive stacked teacher:
    - Batch path: validation-calibrated nonnegative blend over a teacher zoo
      (TabPFN / HGB / RF / ExtraTrees / GBM / Ridge) with fallback to the
      strongest single teacher if blending overfits.
    ...
    def __init__(
        ...
        teacher_min_val_rows=80,
        teacher_top_k=4,
        teacher_blend_ridge=1e-2,
        teacher_score_temperature=0.3,
        use_tabpfn=False,
        tabpfn_device="cpu",
        ...
        teacher_hgb_learning_rate=0.04,
        teacher_rf_estimators=320,
        teacher_rf_min_samples_leaf=1,
        teacher_extra_estimators=320,
        teacher_extra_min_samples_leaf=1,
        teacher_gbm_estimators=220,
        teacher_gbm_max_depth=3,
        teacher_gbm_learning_rate=0.04,
        teacher_ridge_cv=3,
        ...
        self.teacher_extra_estimators = teacher_extra_estimators
        self.teacher_extra_min_samples_leaf = teacher_extra_min_samples_leaf
    ...
    def _fit_teacher(self, X, y):
        ...
        teacher_specs.extend([
            ...
            ("extra", lambda: ExtraTreesRegressor(
                n_estimators=int(self.teacher_extra_estimators),
                min_samples_leaf=int(self.teacher_extra_min_samples_leaf),
                max_features=0.7,
                n_jobs=-1,
                random_state=int(self.random_state),
            )),
        ...
        if "tabpfn" in chosen_names and n <= 600 and p <= 40:
            prior[chosen_names.index("tabpfn")] *= 1.20
        if "ridge" in chosen_names and p >= 35:
            prior[chosen_names.index("ridge")] *= 1.15
        ...
        blend_train_rmse = self._rmse(y, P_train @ weights)
        single_train_rmse = [self._rmse(y, pred) for pred in chosen_train_preds]
        best_single_idx = int(np.argmin(single_train_rmse))
        if blend_train_rmse > single_train_rmse[best_single_idx] * 1.03:
    ...
    def __str__(self):
        ...
        lines = [
            f"Adaptive Stacked Teacher Audit Atlas (teacher={self.teacher_kind_})",
...
AdaptiveStackedTeacherAuditAtlasRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "AdaptiveStackedTeacherAuditAtlas_v1"
model_description = "Dual-path regressor with regime-adaptive stacked teacher blend (HGB/RF/ExtraTrees/GBM/Ridge with optional TabPFN) plus compact single-row audit atlas and exact prompt-answer map"
model_defs = [(model_shorthand_name, AdaptiveStackedTeacherAuditAtlasRegressor())]
