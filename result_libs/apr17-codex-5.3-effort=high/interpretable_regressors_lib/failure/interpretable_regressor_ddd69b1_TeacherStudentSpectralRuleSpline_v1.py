...
class TeacherStudentRuleSplineRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(
        self,
        teacher_n_estimators=120,
        teacher_learning_rate=0.08,
        teacher_max_depth=3,
        teacher_subsample=1.0,
        val_fraction=0.2,
        max_student_features=12,
        max_student_terms=10,
        alpha_student=1e-3,
        corr_screen_rel=0.02,
        hinge_quantiles=(0.15, 0.35, 0.5, 0.65, 0.85),
        interaction_top_features=5,
        candidate_eval_topk=10,
        min_rel_gain=0.002,
        coef_decimals=4,
        coef_tol=1e-10,
        symbolic_n_rows=1,
    ...
    def _screen_features(self, X, y):
        ...
        for j in range(n_features):
            ...
            transforms = [
                x,
                x * x - float(np.mean(x * x)),
                np.abs(x) - float(np.mean(np.abs(x))),
                np.maximum(0.0, x),
                np.maximum(0.0, -x),
                np.sin(x),
                np.cos(x),
                np.exp(-np.abs(np.clip(x, -6.0, 6.0))),
            ]

            best = 0.0
            for t in transforms:
                tc = t - float(np.mean(t))
                denom = (float(np.linalg.norm(tc)) + 1e-12) * y_norm
                c = abs(float(tc @ yc) / denom)
                if c > best:
                    best = c
            scores[j] = best
        ...
        max_corr = float(scores[order[0]])
        if max_corr <= 1e-12:
            return list(range(min(int(self.max_student_features), n_features)))

        selected = []
        for j in order:
            if scores[j] >= float(self.corr_screen_rel) * max_corr:
    ...
    def _term_features(term):
        t = term["type"]
        if t in {"lin", "sq", "cube", "abs", "hinge", "step", "sin", "cos", "expneg"}:
            return {int(term["feature"])}
        if t == "int":
            return {int(term["a"]), int(term["b"])}
        if t == "tri":
            return {int(term["a"]), int(term["b"]), int(term["c"])}
        if t == "gate":
    ...
    def _eval_term(X, term):
        ...
        if t == "sin":
            return np.sin(X[:, int(term["feature"])])
        if t == "cos":
            return np.cos(X[:, int(term["feature"])])
        if t == "expneg":
            col = np.clip(X[:, int(term["feature"])], -8.0, 8.0)
            return np.exp(-np.abs(col))
        if t == "hinge":
    ...
    def _term_text(self, term, dec):
        ...
        if t == "cube":
            return f"(x{int(term['feature'])}^3)"
        if t == "abs":
            return f"abs(x{int(term['feature'])})"
        if t == "sin":
            return f"sin(x{int(term['feature'])})"
        if t == "cos":
            return f"cos(x{int(term['feature'])})"
        if t == "expneg":
            return f"exp(-|x{int(term['feature'])}|)"
        if t == "hinge":
            feat = int(term["feature"])
            knot = float(term["knot"])
            if int(term["direction"]) > 0:
        ...
        if t == "tri":
            return f"(x{int(term['a'])} * x{int(term['b'])} * x{int(term['c'])})"
        if t == "gate":
            gate = int(term["gate"])
            target = int(term["target"])
            knot = float(term["knot"])
    ...
    def _dedupe_terms(terms):
        ...
        seen = set()
        for t in terms:
            if t["type"] in {"lin", "sq", "cube", "abs", "sin", "cos", "expneg"}:
                ...
                a, b, c = sorted((int(t["a"]), int(t["b"]), int(t["c"])))
                key = ("tri", a, b, c)
            elif t["type"] == "gate":
    ...
    def _build_candidates(self, X, screened):
        ...
        for feat in screened:
            ...
            terms.append({"type": "cube", "feature": feat})
            terms.append({"type": "abs", "feature": feat})
            terms.append({"type": "sin", "feature": feat})
            terms.append({"type": "cos", "feature": feat})
            terms.append({"type": "expneg", "feature": feat})
            xcol = X[:, feat]
            knots = [0.0]
        ...
        tri_feats = inter_feats[: min(4, len(inter_feats))]
        for a, b, c in combinations(tri_feats, 3):
            terms.append({"type": "tri", "a": int(a), "b": int(b), "c": int(c)})
    ...
    def __str__(self):
        ...
        lines = [
            "Teacher-Student Spectral Rule-Spline Regressor",
            ...
            "  - To simulate: compute every active term value, multiply by its coefficient, then add intercept.",
            "",
...
model_shorthand_name = "TeacherStudentSpectralRuleSpline_v1"
model_description = "Two-path model with transform-aware screening and sparse symbolic student over linear/polynomial/trig/exp/hinge/interaction terms plus GBM batch teacher"
