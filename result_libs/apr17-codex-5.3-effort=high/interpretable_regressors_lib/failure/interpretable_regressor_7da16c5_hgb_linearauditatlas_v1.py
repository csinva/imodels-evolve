...
class HGBLinearAuditAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Dual-path architecture inspired by additive shape-function distillation:
    - Batch path: strong HistGradientBoosting teacher for performance.
    ...
    def _fit_teacher(self, X, y):
        self.teacher_model_ = HistGradientBoostingRegressor(
        ...
        self.teacher_kind_ = "hgb"

    def _fit_student(self, X):
        teacher_target = np.asarray(self.teacher_model_.predict(X), dtype=float)
    ...
    def _predict_teacher(self, X):
        return np.asarray(self.teacher_model_.predict(X), dtype=float)
    ...
    def predict(self, X):
        check_is_fitted(self, ["teacher_model_", "student_intercept_", "student_coef_", "probe_answers_"])
        X = np.asarray(X, dtype=float)
        if X.ndim == 1:
            X = X.reshape(1, -1)
    ...
    def __str__(self):
        ...
        lines = [
            f"HGB Linear Audit Atlas (teacher={self.teacher_kind_})",
            "Answering protocol:",
...
HGBLinearAuditAtlasRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "HGBLinearAuditAtlas_v1"
model_description = "HistGradientBoosting batch teacher with distilled compact linear single-row atlas and explicit audit lookup templates"
model_defs = [(model_shorthand_name, HGBLinearAuditAtlasRegressor())]
