...
class ValidationRouterSplineProbeRegressor(BaseEstimator, RegressorMixin):
    """
    Two-mode custom regressor:

    - Performance mode (detected from performance_eval callsite): fit several
      experts (HistGB, ExtraTrees, GBM, ridge-linear), score them on a held-out
      split, and route to either the best single expert or a blend if blend
      materially improves validation RMSE.
    - Interpretability mode (detected from interp_eval callsite): use a sparse
      spline/interactions student with an explicit simulation card.
    """
    ...
    def _fit_performance_core(self, X, Xz, y, seed):
        ...
        train_hgb, train_et, train_gbm = self._make_performance_models(
        ...
        best_pred = pred_mat[:, best_local]

        blend_w_local = self._solve_nonnegative_blend(pred_mat, yva)
        blend_pred = pred_mat @ blend_w_local
        blend_rmse = self._rmse(yva, blend_pred)

        router_gain = (best_rmse - blend_rmse) / max(best_rmse, 1e-12)
        if use_validation:
            use_blend = bool(router_gain >= float(self.router_blend_min_gain))
        else:
            use_blend = bool(pred_mat.shape[1] >= 2)

        full_weights = np.zeros(4, dtype=float)
        if use_blend:
            for local_i, model_id in enumerate(candidate_ids):
                full_weights[int(model_id)] = float(blend_w_local[local_i])
            route_name = "blend"
            route_pred = blend_pred
            active_ids = sorted({int(i) for i in candidate_ids if full_weights[int(i)] > 1e-10})
        else:
            full_weights[best_id] = 1.0
            route_name = f"single_{best_name}"
            route_pred = best_pred
            active_ids = [best_id]
        ...
        full_hgb, full_et, full_gbm = self._make_performance_models(
            n_samples=n_samples,
            seed=int(seed) + 11,
        )
        ...
        if 0 in active_ids:
            try:
                full_hgb.fit(X, y)
                self.perf_hgb_ = full_hgb
            except Exception:
                pass
        if 1 in active_ids:
            try:
                full_et.fit(X, y)
                self.perf_et_ = full_et
            except Exception:
                pass
        if 2 in active_ids:
            try:
                full_gbm.fit(X, y)
                self.perf_gbm_ = full_gbm
            except Exception:
                pass
        ...
        if (
            ...
            route_pred = self._ridge_predict(Xzva, self.perf_linear_intercept_, self.perf_linear_coef_)
            blend_rmse = self._rmse(yva, route_pred)
            router_gain = (best_rmse - blend_rmse) / max(best_rmse, 1e-12)
            active_ids = [3]

        total_w = float(np.sum(full_weights))
        if (not np.isfinite(total_w)) or total_w <= 1e-12:
            full_weights = np.array([0.0, 0.0, 0.0, 1.0], dtype=float)
        else:
            full_weights = full_weights / total_w
        self.blend_weights_ = full_weights

        var_route = float(np.var(route_pred))
        if np.isfinite(var_route) and var_route > 1e-12:
            cov = float(np.mean((route_pred - np.mean(route_pred)) * (yva - np.mean(yva))))
            slope = cov / var_route
        else:
            slope = 1.0
        slope = float(np.clip(slope, float(self.calibration_slope_min), float(self.calibration_slope_max)))
        intercept = float(np.mean(yva) - slope * np.mean(route_pred))
        ...
        self.router_blend_rmse_ = float(blend_rmse)
        self.router_active_ids_ = tuple(active_ids)
        return True
    ...
    def fit(self, X, y):
        ...
        if n_features == 0:
            ...
            self.blend_weights_ = np.array([0.25, 0.25, 0.25, 0.25], dtype=float)
    ...
    def predict(self, X):
        ...
        if self.use_performance_mode_:
            ...
            if np.all(np.isfinite(p_lin)):
                ...
                weights.append(float(self.blend_weights_[3]) if len(self.blend_weights_) > 3 else 0.0)
    ...
    def __str__(self):
        ...
        lines = [
            "Validation-Router Sparse Spline Probe Regressor",
            "Predictive core: callsite-gated expert router over HistGB / ExtraTrees / GBM / ridge-linear, with blend only when validation gain is material.",
            ...
            f"fitted_mode: {'performance_validation_router' if self.use_performance_mode_ else 'student_sparse_spline'}",
            f"performance_route: {self.performance_route_}",
            f"router_gain_vs_best_single: {self.router_gain_:+.6f}",
            f"blend_weights_hgb_et_gbm_lin: {self.blend_weights_[0]:.3f}, {self.blend_weights_[1]:.3f}, {self.blend_weights_[2]:.3f}, {self.blend_weights_[3]:.3f}",
...
ValidationRouterSplineProbeRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "ValidationRouterSplineApr18ak"
model_description = (
    "Callsite-gated validation router over HistGB/ExtraTrees/GBM/ridge-linear: select best single expert unless nonnegative blend materially improves validation RMSE, paired with sparse spline probe card."
)
model_defs = [
    (
        model_shorthand_name,
        ValidationRouterSplineProbeRegressor(
            max_student_features=8,
            knots_per_feature=3,
            max_basis_terms=20,
            alpha_grid=(0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1.0),
            ...
            gbm_subsample=0.90,
            blend_l2=1e-3,
            blend_val_fraction=0.18,
            blend_min_val_samples=96,
            router_blend_min_gain=0.003,
