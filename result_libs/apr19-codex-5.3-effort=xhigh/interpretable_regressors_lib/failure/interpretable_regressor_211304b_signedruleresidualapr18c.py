...
class SignedRuleResidualRegressor(BaseEstimator, RegressorMixin):
    """
    Sparse residual-rule regressor with a directly simulatable equation.

    Structure:
      1) Sparse linear backbone on raw features.
      2) Greedy residual corrections from a tiny nonlinear library:
         max(0, xj), max(0, -xj), xj^2, I(xj>0), and limited pairwise products.
    """
    ...
    def __init__(
        ...
        self.min_linear_coef = min_linear_coef
    ...
    def _eval_term(self, X, term):
        ...
        if kind == "quadratic":
            return X[:, term[1]] ** 2
        if kind == "step_gt_zero":
            return (X[:, term[1]] > 0.0).astype(float)
        if kind == "interaction":
    ...
    def _term_to_str(self, term):
        ...
        if kind == "quadratic":
            return f"(x{term[1]}^2)"
        if kind == "step_gt_zero":
            return f"I(x{term[1]} > 0)"
        if kind == "interaction":
            return f"(x{term[1]} * x{term[2]})"
        return str(term)

    def _build_nonlinear_library(self, X, ranked_features):
        n_samples, n_features = X.shape
        if n_features == 0:
            return [], np.zeros((n_samples, 0), dtype=float)

        top = list(ranked_features[: min(self.nonlinear_pool, len(ranked_features))])
        terms = []
        cols = []
        ...
        for j in top:
            j = int(j)
            xj = X[:, j]
            terms.append(("relu_pos", j))
            cols.append(np.maximum(0.0, xj))

            terms.append(("relu_neg", j))
            cols.append(np.maximum(0.0, -xj))
            ...
            if self.include_steps:
                terms.append(("step_gt_zero", j))
                cols.append((xj > 0.0).astype(float))

        inter_top = list(ranked_features[: min(self.interaction_pool, len(ranked_features))])
    ...
    def fit(self, X, y):
        ...
        # Rank linear effects via ridge on standardized features, then convert back to raw-x scale.
        ...
        raw_linear = w_std / x_scale

        ranked = np.argsort(np.abs(raw_linear))[::-1]
        linear_feats = [
            int(j) for j in ranked if abs(raw_linear[j]) >= self.min_linear_coef
        ][: min(self.max_linear_terms, n_features)]
        if not linear_feats:
            linear_feats = [int(ranked[0])]

        current_terms = [("linear", j) for j in linear_feats]
        current_cols = [X[:, j] for j in linear_feats]
        ...
        nonlinear_terms, Phi_nonlin = self._build_nonlinear_library(X, linear_feats)
        remaining = list(range(Phi_nonlin.shape[1]))

        while len(current_terms) < self.max_terms and remaining:
            scores = np.array(
                [abs(self._safe_center_corr(Phi_nonlin[:, j], residual)) for j in remaining],
                dtype=float,
            )
            ...
            trial_cols = current_cols + [Phi_nonlin[:, best_j]]
            ...
            current_terms.append(nonlinear_terms[best_j])
        ...
        if np.any(keep):
            ...
            best = int(np.argmax(np.abs(coefs)))
            current_terms = [current_terms[best]]
            current_cols = [current_cols[best]]
        ...
        for c, t in zip(self.coef_, self.terms_):
            w = float(abs(c))
            if t[0] == "interaction":
                importance[t[1]] += 0.5 * w
                importance[t[2]] += 0.5 * w
            else:
                importance[t[1]] += w
    ...
    def __str__(self):
        ...
        lines = [
            "Signed Rule Residual Regressor",
            "Prediction uses raw features directly (no normalization).",
            "",
            "Equation:",
        ]
        ...
        for c, t in zip(self.coef_, self.terms_):
            lines.append(f"  {c:+.6f} * {self._term_to_str(t)}")
...
SignedRuleResidualRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "SignedRuleResidualApr18c"
model_description = (
    "Sparse raw-feature linear backbone plus greedy residual signed rules "
    "(relu/step/quadratic/interaction) for compact, directly simulatable equations."
)
model_defs = [
    (
        model_shorthand_name,
        SignedRuleResidualRegressor(
            max_terms=8,
            max_linear_terms=6,
            nonlinear_pool=4,
            interaction_pool=3,
            min_relative_improvement=9e-4,
            prune_threshold=4e-4,
        ),
    )
]
