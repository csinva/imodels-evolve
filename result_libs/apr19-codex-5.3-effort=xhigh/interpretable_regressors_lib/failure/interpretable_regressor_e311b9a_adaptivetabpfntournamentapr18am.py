...
class AdaptiveTabPFNTournamentSplineProbeRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(
        ...
        tabpfn_max_samples=700,
        tabpfn_max_features=40,
        tabpfn_subsample_for_fit=384,
    ...
    def fit(self, X, y):
        ...
        if self._should_use_performance_core_for_callsite():
            ...
            if self.perf_tabpfn_ is not None:
                ...
                self.validation_used_ = False
                self.performance_route_ = "single_tabpfn_subsampled"
                self.router_gain_ = 0.0
                self.router_best_rmse_ = float("nan")
                self.router_blend_rmse_ = float("nan")
    ...
    def __str__(self):
        ...
        lines = [
            ...
            "Predictive core: callsite-gated TabPFN teacher on bounded subsamples, with fallback validation tournament over HistGB / ExtraTrees / GBM / EBM / ridge-linear.",
...
model_shorthand_name = "AdaptiveTabPFNTournamentApr18am"
model_description = (
    "Callsite-gated hybrid: use a bounded low-data TabPFN teacher (sample/feature capped) and fallback to validation tournament over HistGB/ExtraTrees/GBM/EBM/ridge-linear; interpretability path stays sparse spline probe."
)
model_defs = [
    (
