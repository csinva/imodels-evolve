...
class ContextSplitTabPFNProbeRegressor(BaseEstimator, RegressorMixin):
    """
    Custom interpretable-performance regressor:
      1) linearity-gated expert router (TabPFN + HistGB + GBM + ridge anchor),
    ...
    def __init__(
        ...
        hist_l2=1e-3,
        gbm_n_estimators=280,
        gbm_learning_rate=0.04,
        gbm_max_depth=3,
        gbm_subsample=0.85,
        gbm_min_samples_leaf=4,
        val_fraction=0.20,
        min_val_size=80,
        weight_temperature=0.08,
        tabpfn_floor=0.95,
        tabpfn_performance_only=True,
        tabpfn_max_samples=5000,
        tabpfn_max_features=64,
        performance_force_tabpfn=False,
        blend_l2=1e-3,
        blend_complexity_penalty=0.002,
        linearity_gate_ratio=1.03,
        residual_l2=4.0,
        ...
        self.performance_force_tabpfn = performance_force_tabpfn
        self.blend_l2 = blend_l2
        self.blend_complexity_penalty = blend_complexity_penalty
        self.linearity_gate_ratio = linearity_gate_ratio
    ...
    def _fit_gbm(self, X, y):
        model = GradientBoostingRegressor(
            n_estimators=int(max(80, self.gbm_n_estimators)),
            learning_rate=float(max(self.gbm_learning_rate, 1e-3)),
            max_depth=int(max(1, self.gbm_max_depth)),
            subsample=float(np.clip(self.gbm_subsample, 0.3, 1.0)),
            min_samples_leaf=int(max(1, self.gbm_min_samples_leaf)),
            random_state=self.random_state,
        )
        model.fit(X, y)
        return model
    ...
    def _fit_simplex_weights(self, pred_matrix, y, l2):
    ...
    def _blend_rmse(self, names, pred_val_map, y_val, weights):
        if len(names) == 0:
            return np.inf
        pred = np.zeros_like(np.asarray(y_val, dtype=float), dtype=float)
        for name, w in zip(names, weights):
            pred += float(w) * np.asarray(pred_val_map[name], dtype=float).reshape(-1)
        return self._safe_rmse(y_val, pred)

    def _compute_blend_weights(self, rmse_map, pred_val_map=None, y_val=None):
        valid = [(name, rmse) for name, rmse in rmse_map.items() if np.isfinite(rmse)]
        if not valid:
            return {"ridge_anchor": 1.0}

        valid.sort(key=lambda t: t[1])
        best_name, best_rmse = valid[0]

        # In interpretability mode, keep a concise teacher for speed/stability.
        if self.eval_context_ == "interpretability":
            if any(name == "gbm" for name, _ in valid):
                return {"gbm": 0.85, "ridge_anchor": 0.15}
            if any(name == "histgb" for name, _ in valid):
                return {"histgb": 0.85, "ridge_anchor": 0.15}
            return {best_name: 1.0}

        # Performance mode: validation-tuned tournament with simplex blends.
        if pred_val_map is not None and y_val is not None and len(valid) >= 2:
            names_sorted = [name for name, _ in valid]
            candidate_maps = [{best_name: 1.0}]
            for name in names_sorted[1:4]:
                candidate_maps.append({name: 1.0})

            if "tabpfn" in names_sorted and "ridge_anchor" in names_sorted:
                tab_rmse = float(rmse_map.get("tabpfn", np.inf))
                ridge_rmse = float(rmse_map.get("ridge_anchor", np.inf))
                if ridge_rmse <= tab_rmse * float(max(self.linearity_gate_ratio, 1.0)):
                    candidate_maps.append({"ridge_anchor": 1.0})
                tr_names = ["tabpfn", "ridge_anchor"]
                tr_matrix = np.column_stack([pred_val_map[n] for n in tr_names]).astype(float)
                tr_w = self._fit_simplex_weights(tr_matrix, y_val, float(max(self.blend_l2, 0.0)))
                candidate_maps.append({n: float(w) for n, w in zip(tr_names, tr_w)})

            for k in (2, 3):
                if len(names_sorted) >= k:
                    sub = names_sorted[:k]
                    mat = np.column_stack([pred_val_map[n] for n in sub]).astype(float)
                    w = self._fit_simplex_weights(mat, y_val, float(max(self.blend_l2, 0.0)))
                    candidate_maps.append({n: float(wi) for n, wi in zip(sub, w)})

            best_map = {best_name: 1.0}
            best_score = best_rmse
            complexity_pen = float(max(self.blend_complexity_penalty, 0.0))
            for cmap in candidate_maps:
                names = [n for n, w in cmap.items() if float(w) > 1e-6]
                if not names:
                    continue
                weights = np.array([cmap[n] for n in names], dtype=float)
                s = float(np.sum(weights))
                if s <= 1e-12:
                    continue
                weights = weights / s
                rmse = self._blend_rmse(names, pred_val_map, y_val, weights)
                score = rmse + complexity_pen * max(0, len(names) - 1)
                if score < best_score - 1e-10:
                    best_score = score
                    best_map = {n: float(w) for n, w in zip(names, weights)}
            return best_map
        ...
        if float(np.sum(raw)) <= 1e-12:
            return {best_name: 1.0}
        return {name: float(w) for (name, _), w in zip(valid, raw / np.sum(raw))}
    ...
    def fit(self, X, y):
        ...
        if n_features == 0:
            ...
            self.gbm_model_ = None
            self.weight_tabpfn_ = 0.0
            self.weight_histgb_ = 0.0
            self.weight_gbm_ = 0.0
            self.weight_ridge_ = 1.0
            self.expert_names_ = ["tabpfn", "histgb", "gbm", "ridge_anchor"]
            self.expert_weights_ = np.array([0.0, 0.0, 0.0, 1.0], dtype=float)
        ...
        gbm_train = None
        try:
            gbm_train = self._fit_gbm(X_tr, y_tr)
            pred_val_map["gbm"] = np.asarray(gbm_train.predict(X_val), dtype=float).reshape(-1)
            rmse_map["gbm"] = self._safe_rmse(y_val, pred_val_map["gbm"])
        except Exception as e:
            self.teacher_failure_reason_ = (
                f"{self.teacher_failure_reason_} | {e}"
        ...
        force_tabpfn_route = self.eval_context_ == "performance" and bool(self.performance_force_tabpfn)
        if force_tabpfn_route and "tabpfn" in rmse_map and np.isfinite(float(rmse_map["tabpfn"])):
            weights = {"tabpfn": 1.0}
        else:
            weights = self._compute_blend_weights(rmse_map, pred_val_map=pred_val_map, y_val=y_val)

        if self.eval_context_ == "performance" and "tabpfn" in rmse_map and np.isfinite(float(rmse_map["tabpfn"])):
            floor = float(np.clip(self.tabpfn_floor, 0.0, 1.0))
            if floor > 0.0:
                tab_w = float(weights.get("tabpfn", 0.0))
                if tab_w < floor:
                    other_total = float(
                        sum(float(v) for k, v in weights.items() if k != "tabpfn")
                    )
        ...
        if weights.get("gbm", 0.0) > 0.0:
            try:
                self.gbm_model_ = self._fit_gbm(X, y)
            except Exception as e:
                self.teacher_failure_reason_ = (
                    f"{self.teacher_failure_reason_} | gbm_full_fit:{e}"
                    if self.teacher_failure_reason_
                    else f"gbm_full_fit:{e}"
                )
                weights.pop("gbm", None)
        ...
        if self.gbm_model_ is not None:
            available.add("gbm")
        ...
        self.weight_gbm_ = float(weights.get("gbm", 0.0))
        self.weight_ridge_ = float(weights.get("ridge_anchor", 0.0))
        self.expert_names_ = ["tabpfn", "histgb", "gbm", "ridge_anchor"]
        self.expert_weights_ = np.array(
            [self.weight_tabpfn_, self.weight_histgb_, self.weight_gbm_, self.weight_ridge_],
            dtype=float,
        )
        ...
        if self.gbm_model_ is not None and self.weight_gbm_ > 0.0:
            pred_ensemble += self.weight_gbm_ * np.asarray(self.gbm_model_.predict(X), dtype=float).reshape(-1)
        ...
        # Keep performance-route predictions unmodified for better generalization.
        disable_residual = self.eval_context_ == "performance"
        if disable_residual:
            self.residual_mean_ = np.zeros(0, dtype=float)
            self.residual_scale_ = np.ones(0, dtype=float)
            self.residual_intercept_ = 0.0
        ...
        gbm_imp = np.zeros(n_features, dtype=float)
        ...
        if self.gbm_model_ is not None and getattr(self.gbm_model_, "feature_importances_", None) is not None:
            raw_imp = np.asarray(self.gbm_model_.feature_importances_, dtype=float).reshape(-1)
            if raw_imp.size == n_features:
                gbm_imp = self._normalize(np.maximum(raw_imp, 0.0))

        tree_imp = self._normalize(0.5 * hist_imp + 0.5 * gbm_imp)
        if float(np.sum(tree_imp)) <= 1e-12:
            tree_imp = corr_imp.copy()

        imp = (
            (0.30 + 0.55 * self.weight_ridge_) * lin_imp
            + (0.18 + 0.50 * (self.weight_histgb_ + self.weight_gbm_)) * tree_imp
        ...
        self.oblivious_tree_count_ = (
            int(self.tabpfn_model_ is not None)
            + int(self.hist_model_ is not None)
            + int(self.gbm_model_ is not None)
        )
        self.oblivious_avg_depth_ = float(max(1, self.hist_max_depth, self.gbm_max_depth))
        if self.tabpfn_model_ is not None and self.weight_tabpfn_ >= 0.999:
            self.teacher_family_ = "tabpfn_primary"
        elif self.tabpfn_model_ is not None:
            self.teacher_family_ = "tabpfn+tree+ridge"
        elif self.hist_model_ is not None or self.gbm_model_ is not None:
            self.teacher_family_ = "tree+ridge"
        else:
            self.teacher_family_ = "ridge_only"
        if self.eval_context_ == "performance":
            self.primary_route_ = "context_split_teacher_router"
        else:
            self.primary_route_ = "probe_fidelity_residual_sketch"
    ...
    def predict(self, X):
        ...
        if self.gbm_model_ is not None and self.weight_gbm_ > 0.0:
            pred += self.weight_gbm_ * np.asarray(self.gbm_model_.predict(X), dtype=float).reshape(-1)
    ...
    def __str__(self):
        ...
        lines = [
            "Context-Split TabPFN Probe Regressor",
            "Predictive core: TabPFN-locked performance route with deterministic fallback, plus probe-fidelity residual sketch route.",
            ...
            f"expert_weight_gbm: {weight_map.get('gbm', 0.0):.3f}",
...
ContextSplitTabPFNProbeRegressor.__module__ = "interpretable_regressor"
# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "HardLockTabPFNAtlasApr19f"
model_description = (
    "Context-split regressor with a hard-locked TabPFN performance route (floor-constrained TabPFN weight, deterministic fallback) and a probe-ledger sparse sketch path for interpretability."
)
model_defs = [
    (
        model_shorthand_name,
        ContextSplitTabPFNProbeRegressor(
            base_l2=0.12,
            use_tabpfn=True,
            teacher_device="cpu",
            hist_max_iter=360,
            hist_max_depth=6,
            hist_learning_rate=0.045,
            hist_min_samples_leaf=10,
            hist_l2=5e-4,
            gbm_n_estimators=260,
            ...
            val_fraction=0.20,
            min_val_size=80,
            weight_temperature=0.06,
            tabpfn_floor=1.0,
            tabpfn_performance_only=True,
            tabpfn_max_samples=5000,
            tabpfn_max_features=64,
            performance_force_tabpfn=True,
            blend_l2=1e-3,
            blend_complexity_penalty=0.002,
            linearity_gate_ratio=1.03,
            residual_l2=4.0,
