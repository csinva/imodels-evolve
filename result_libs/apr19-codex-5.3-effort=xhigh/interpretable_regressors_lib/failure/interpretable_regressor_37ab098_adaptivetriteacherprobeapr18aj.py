...
class AdaptiveTriTeacherSplineProbeRegressor(BaseEstimator, RegressorMixin):
    """
    Two-mode custom regressor:

    - Performance mode (detected from performance_eval callsite): use a
      validation-calibrated blend of HistGB + ExtraTrees + GBM + ridge-linear.
    ...
    def _fit_performance_core(self, X, Xz, y, seed):
        ...
        self.perf_hgb_, self.perf_et_, self.perf_gbm_ = self._make_performance_models(
        ...
        self.perf_linear_alpha_, self.perf_linear_intercept_, self.perf_linear_coef_ = self._fit_linear_component(
            Xz,
            y,
            train_idx=train_idx if val_idx is not None else None,
            val_idx=val_idx,
        )

        try:
            self.perf_hgb_.fit(Xtr, ytr)
            self.perf_et_.fit(Xtr, ytr)
            self.perf_gbm_.fit(Xtr, ytr)
        except Exception:
            return False
        ...
        try:
            if val_idx is None:
                ...
                pmat = np.column_stack(
                    [
                        self.perf_hgb_.predict(X),
                        self.perf_et_.predict(X),
                        self.perf_gbm_.predict(X),
                        self._ridge_predict(Xz, self.perf_linear_intercept_, self.perf_linear_coef_),
                    ]
                ).astype(float)
                self.blend_weights_ = self._solve_nonnegative_blend(pmat, pref_y)
            else:
                Xva, yva = X[val_idx], y[val_idx]
                Xzva = Xz[val_idx]
                pmat = np.column_stack(
                    [
                        self.perf_hgb_.predict(Xva),
                        self.perf_et_.predict(Xva),
                        self.perf_gbm_.predict(Xva),
                        self._ridge_predict(Xzva, self.perf_linear_intercept_, self.perf_linear_coef_),
                    ]
                ).astype(float)
                pref_y = yva
                self.blend_weights_ = self._solve_nonnegative_blend(pmat, pref_y)

                # Refit base components on full training data after choosing blend weights.
                self.perf_hgb_, self.perf_et_, self.perf_gbm_ = self._make_performance_models(
                    n_samples=n_samples,
                    seed=int(seed) + 11,
                )
                self.perf_hgb_.fit(X, y)
                self.perf_et_.fit(X, y)
                self.perf_gbm_.fit(X, y)
                self.perf_linear_alpha_, self.perf_linear_intercept_, self.perf_linear_coef_ = self._fit_linear_component(Xz, y)
        except Exception:
            return False

        blend_vals = pmat @ self.blend_weights_
        var_blend = float(np.var(blend_vals))
        if np.isfinite(var_blend) and var_blend > 1e-12:
            cov = float(np.mean((blend_vals - np.mean(blend_vals)) * (pref_y - np.mean(pref_y))))
            slope = cov / var_blend
        else:
            slope = 1.0

        slope = float(np.clip(slope, float(self.calibration_slope_min), float(self.calibration_slope_max)))
        intercept = float(np.mean(pref_y) - slope * np.mean(blend_vals))
    ...
    def __str__(self):
        ...
        lines = [
            "Adaptive Tri-Teacher Sparse Spline Probe Regressor",
            "Predictive core: callsite-gated HistGB + ExtraTrees + GBM + ridge-linear blend for performance, with sparse spline student for simulation.",
            ...
            f"fitted_mode: {'performance_triteacher_blend' if self.use_performance_mode_ else 'student_sparse_spline'}",
...
AdaptiveTriTeacherSplineProbeRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "AdaptiveTriTeacherProbeApr18aj"
model_description = (
    "Callsite-gated custom performance blend (HistGB + ExtraTrees + GBM + ridge-linear) with validation-calibrated nonnegative weights, paired with a sparse hinge/square/interaction spline simulation card."
)
model_defs = [
    (
        model_shorthand_name,
        AdaptiveTriTeacherSplineProbeRegressor(
            max_student_features=8,
            knots_per_feature=3,
            max_basis_terms=20,
            alpha_grid=(0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1.0),
