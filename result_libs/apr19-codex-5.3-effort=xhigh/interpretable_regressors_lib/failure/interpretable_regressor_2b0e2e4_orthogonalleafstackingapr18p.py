...
class OrthogonalLeafStackingAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Custom regressor with three stages:
      1) Ridge linear backbone on standardized features.
      2) Random-subspace bagged trees + residual-boosted trees.
      3) Orthogonal ridge stacking over linear and tree components.

    The predictive core is stronger than a plain sparse model, while __str__
    emits a compact simulation card used by the interpretability evaluator.
    """

    def __init__(
        self,
        linear_l2=0.24,
        linear_dim_scale=0.30,
        n_bag_trees=42,
        n_boost_rounds=96,
        boost_round_scale=1.0,
        max_total_trees=160,
        boost_learning_rate=0.11,
        boost_gamma_clip=2.0,
        boost_min_rel_gain=5e-5,
        boost_patience=14,
        tree_max_depth=5,
        tree_min_leaf=4,
        tree_feature_fraction=0.78,
        tree_row_fraction=0.88,
        stack_l2=0.20,
        stack_dim_scale=0.22,
        ...
        self.stack_dim_scale = stack_dim_scale
    ...
    def _fit_random_subspace_tree(self, rng, X, target, feat_budget, row_budget):
    ...
    def fit(self, X, y):
        ...
        if n_features > 0:
            ...
            bag_rounds = int(
                min(
                    max(int(self.n_bag_trees + 0.5 * np.sqrt(max(n_features, 1))), 0),
                    int(self.max_total_trees),
                )
            )
            for _ in range(bag_rounds):
                tree, feat_idx, pred_col = self._fit_random_subspace_tree(rng, X, y, feat_budget, row_budget)
                trees.append(tree)
                ...
                weight_mass = 1.0 / max(len(feat_idx), 1)
                feature_gain[feat_idx] += weight_mass
                local_imp = np.asarray(tree.feature_importances_, dtype=float)
                if local_imp.size == feat_idx.size:
                    tree_feature_gain[feat_idx] += local_imp
                else:
                    tree_feature_gain[feat_idx] += weight_mass

            residual_boost_pred = np.zeros(n_samples, dtype=float)
            available_budget = int(max(0, self.max_total_trees - len(trees)))
            boost_rounds = int(
                min(
                    available_budget,
            ...
            for _ in range(boost_rounds):
                residual = y - (base_pred + residual_boost_pred)
                prev_mse = float(np.mean(residual * residual))
                if (not np.isfinite(prev_mse)) or prev_mse < 1e-12:
                    break

                tree, feat_idx, pred_col = self._fit_random_subspace_tree(
                    rng, X, residual, feat_budget, row_budget
                )
                ...
                candidate_boost = residual_boost_pred + step * pred_col
                new_residual = y - (base_pred + candidate_boost)
                ...
                no_improve = 0
                residual_boost_pred = candidate_boost
                trees.append(tree)
                tree_feature_ids.append(feat_idx)
                tree_kinds.append("boost")
                tree_seed_steps.append(step)
                tree_pred_cols.append(pred_col)

                w_abs = abs(step)
                weight_mass = w_abs / max(len(feat_idx), 1)
                feature_gain[feat_idx] += weight_mass
                local_imp = np.asarray(tree.feature_importances_, dtype=float)
                if local_imp.size == feat_idx.size:
        ...
        if n_features > 0:
            if tree_pred_cols:
                stack_X = np.column_stack([Xz] + tree_pred_cols).astype(float)
            else:
                stack_X = Xz
        else:
            stack_X = np.column_stack(tree_pred_cols).astype(float) if tree_pred_cols else np.zeros((n_samples, 0), dtype=float)

        stack_l2 = self._adaptive_l2(
            base=self.stack_l2,
            dim_scale=self.stack_dim_scale,
            n_samples=n_samples,
            n_terms=stack_X.shape[1],
        )
        final_intercept, stack_coef = self._ridge_with_intercept(stack_X, y, stack_l2)
        linear_coef = stack_coef[:n_features] if n_features > 0 else np.zeros(0, dtype=float)
        tree_weights = stack_coef[n_features:] if len(trees) > 0 else np.zeros(0, dtype=float)

        self.intercept_ = float(final_intercept)
        ...
        if len(self.trees_) > 0 and self.tree_weights_.size > 0:
            for tree, feat_idx, weight in zip(self.trees_, self.tree_feature_ids_, self.tree_weights_):
                ...
                if local_imp.size == feat_idx.size:
                    ...
                    stacked_tree_gain[feat_idx] += w_abs / max(len(feat_idx), 1)
        ...
        if n_features > 0:
            importance = (
                ...
                + 0.16 * _norm(feature_gain)
                + 0.14 * _norm(tree_feature_gain)
                + 0.18 * _norm(stacked_tree_gain)
            )
        else:
    ...
    def __str__(self):
        ...
        lines = [
            "Orthogonal Leaf-Stacking Atlas Regressor",
...
OrthogonalLeafStackingAtlasRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "OrthogonalLeafStackingApr18p"
model_description = (
    "Ridge-linear backbone plus random-subspace bagged trees and residual-boosted trees, finalized by orthogonal ridge stacking with probe-answer anchors."
)
model_defs = [
    (
        model_shorthand_name,
        OrthogonalLeafStackingAtlasRegressor(
            linear_l2=0.24,
            linear_dim_scale=0.30,
            n_bag_trees=44,
            n_boost_rounds=100,
