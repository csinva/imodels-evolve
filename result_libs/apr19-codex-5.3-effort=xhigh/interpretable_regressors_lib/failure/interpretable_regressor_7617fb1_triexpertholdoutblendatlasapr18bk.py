...
class TriExpertHoldoutBlendProbeAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Custom tri-expert regressor:
      1) Ridge backbone on standardized features.
      2) Gradient-boosting expert on raw features.
      3) Histogram-gradient-boosting expert on raw features.

    Experts are combined by a nonnegative holdout-gated blend with fallback to
    the best single expert, then a light affine calibration. __str__ exposes a
    compact probe-answer atlas for direct simulation.
    """
    ...
    def __init__(
        ...
        blend_l2=6e-4,
        blend_prior=(0.42, 0.43, 0.15),  # (hist, gbm, ridge)
        blend_floor=(0.08, 0.08, 0.05),
        blend_margin=0.003,
        ...
        self.hist_leaf_nodes = hist_leaf_nodes
    ...
    def fit(self, X, y):
        ...
        if n_features == 0:
            ...
            self.primary_route_ = "validation_gated_tri_expert_blend"
            self.expert_names_ = ["hist_boost", "classic_boost", "ridge_backbone"]
        ...
        gbm_rounds = int(
            min(
                max(int(self.n_rounds + self.round_scale * np.sqrt(max(n_samples, 1))), 40),
                int(max(self.max_rounds_cap, 40)),
            )
        )
        hist_rounds = int(np.clip(int(0.9 * gbm_rounds), 40, 240))
        min_leaf = int(max(self.min_leaf, 2))
        subsample = float(np.clip(self.row_subsample, 0.55, 1.0))

        gbm_params = dict(
            loss="squared_error",
            n_estimators=gbm_rounds,
            learning_rate=float(np.clip(self.learning_rate, 0.01, 0.3)),
            max_depth=int(max(self.depth, 2)),
            min_samples_leaf=min_leaf,
            subsample=subsample,
        )
        hgb_params = dict(
            loss="squared_error",
            learning_rate=float(np.clip(1.05 * self.learning_rate, 0.01, 0.3)),
            max_iter=hist_rounds,
            max_depth=int(max(self.depth + 1, 2)),
            max_leaf_nodes=int(np.clip(self.hist_leaf_nodes, 8, 127)),
            min_samples_leaf=min_leaf,
            l2_regularization=float(max(1e-8, 0.15 * l2)),
        ...
        def _solve_nonnegative_weights(P, target):
            k = P.shape[1]
            lhs = P.T @ P + float(max(self.blend_l2, 0.0)) * np.eye(k, dtype=float)
            rhs = P.T @ target
            try:
                w = np.linalg.solve(lhs, rhs)
            except np.linalg.LinAlgError:
                w = np.linalg.lstsq(P, target, rcond=None)[0]
        ...
        blend_weights = _solve_nonnegative_weights(
            np.column_stack([pred_linear, pred_linear, pred_linear]),
            y,
        )
        min_val = int(max(self.min_validation_samples, 8))
        can_holdout = n_samples >= (2 * min_val + 6)
        if can_holdout:
            rng = np.random.RandomState(int(self.random_state) + 17)
            ...
            if tr_idx.size >= min_val and val_idx.size >= min_val:
                linear_intercept_tr, linear_coef_tr = self._ridge_with_intercept(Xz[tr_idx], y[tr_idx], l2)
                p_linear_val = self._ridge_predict(Xz[val_idx], linear_intercept_tr, linear_coef_tr)

                gbm_val = GradientBoostingRegressor(
                    random_state=int(self.random_state) + 29,
                    **gbm_params,
                )
                ...
                gbm_val.fit(X[tr_idx], y[tr_idx])
                hgb_val.fit(X[tr_idx], y[tr_idx])

                p_gbm_val = np.asarray(gbm_val.predict(X[val_idx]), dtype=float)
                p_hgb_val = np.asarray(hgb_val.predict(X[val_idx]), dtype=float)
                P_val = np.column_stack([p_hgb_val, p_gbm_val, p_linear_val]).astype(float)

                candidate = _solve_nonnegative_weights(P_val, y[val_idx])
                blend_val = P_val @ candidate
                blend_rmse = _safe_rmse(y[val_idx], blend_val)
                expert_rmses = np.array(
                    [
                        _safe_rmse(y[val_idx], p_hgb_val),
                        _safe_rmse(y[val_idx], p_gbm_val),
                        _safe_rmse(y[val_idx], p_linear_val),
                    ],
                    dtype=float,
                )
        ...
        pred_gbm = np.asarray(gbm_model.predict(X), dtype=float)
        pred_hgb = np.asarray(hgb_model.predict(X), dtype=float)
        pred = (
            float(blend_weights[0]) * pred_hgb
            + float(blend_weights[1]) * pred_gbm
            + float(blend_weights[2]) * pred_linear
        )
        ...
        self.gbm_model_ = gbm_model
        self.hgb_model_ = hgb_model
        self.blend_weights_ = np.asarray(blend_weights, dtype=float)
        self.oblivious_trees_ = []
        self.oblivious_tree_count_ = int(gbm_rounds + hist_rounds)
        self.oblivious_avg_depth_ = float(max(self.depth, 2))
        gbm_imp_raw = np.asarray(
            getattr(gbm_model, "feature_importances_", np.zeros(n_features, dtype=float)),
            dtype=float,
        )
        hgb_imp_raw = np.asarray(
            getattr(hgb_model, "feature_importances_", np.zeros(n_features, dtype=float)),
            dtype=float,
        )
        if hgb_imp_raw.shape[0] != n_features:
            hgb_imp_raw = np.zeros(n_features, dtype=float)
        self.tree_feature_gain_ = gbm_imp_raw + 0.75 * hgb_imp_raw
        ...
        gbm_imp = _normalize(gbm_imp_raw)
        hgb_imp = _normalize(hgb_imp_raw)
        ...
        if np.max(tree_imp) > 1e-12:
            imp = 0.33 * gbm_imp + 0.30 * hgb_imp + 0.22 * lin_imp + 0.15 * corr_imp_y
        else:
        ...
        self.expert_names_ = ["hist_boost", "classic_boost", "ridge_backbone"]
        self.expert_weights_ = np.asarray(self.blend_weights_, dtype=float)
        expert_sum = float(np.sum(self.expert_weights_))
        if not np.isfinite(expert_sum) or expert_sum <= 1e-12:
            self.expert_weights_ = np.array([0.0, 1.0, 0.0], dtype=float)
        else:
            self.expert_weights_ = self.expert_weights_ / expert_sum
    ...
    def predict(self, X):
        ...
        p_gbm = (
            np.asarray(self.gbm_model_.predict(X), dtype=float)
        ...
        pred = w[0] * p_hgb + w[1] * p_gbm + w[2] * p_linear
    ...
    def __str__(self):
        ...
        lines = [
            "Tri-Expert Holdout Blend Probe Atlas Regressor",
            "Predictive core: ridge backbone + gradient-boosting + hist-gradient-boosting with holdout-gated nonnegative blending.",
            ...
            f"expert_weight_classic_boost: {weight_map.get('classic_boost', 0.0):.3f}",
...
TriExpertHoldoutBlendProbeAtlasRegressor.__module__ = "interpretable_regressor"
# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "TriExpertHoldoutBlendAtlasApr18bk"
model_description = (
    "Validation-gated tri-expert blend: ridge backbone + gradient boosting + histogram gradient boosting with nonnegative holdout weighting and probe-answer simulation atlas."
)
model_defs = [
    (
        model_shorthand_name,
        TriExpertHoldoutBlendProbeAtlasRegressor(
            ridge_l2=0.18,
            n_rounds=128,
            max_rounds_cap=256,
            depth=3,
