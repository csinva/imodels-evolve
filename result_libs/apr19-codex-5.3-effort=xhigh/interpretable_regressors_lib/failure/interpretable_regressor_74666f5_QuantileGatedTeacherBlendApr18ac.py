...
class QuantileGatedTeacherBlendAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Quantile-gated teacher blend:
      ...
      4) RandomForest teacher on raw features.
      5) Nonnegative global blend, then per-quantile gated local blend on the
         dominant feature, followed by affine calibration.
    ...
    def __init__(
        ...
        gbm_estimators_base=140,
        gbm_estimators_scale=5.0,
        gbm_estimators_cap=340,
        gbm_learning_rate=0.045,
        gbm_max_depth=3,
        gbm_subsample=0.85,
        gbm_min_samples_leaf=6,
        hgb_iter_base=130,
        hgb_iter_scale=5.0,
        hgb_iter_cap=320,
        ...
        rf_estimators_base=120,
        rf_estimators_scale=4.5,
        rf_estimators_cap=320,
        rf_max_depth=None,
        rf_min_samples_leaf=2,
        rf_max_features=0.8,
        gating_bins=3,
        gating_min_bin_samples=72,
        gating_shrinkage=36.0,
        ...
        self.gating_bins = gating_bins
        self.gating_min_bin_samples = gating_min_bin_samples
        self.gating_shrinkage = gating_shrinkage
    ...
    def _compute_bin_cuts(self, x_col):
        n_bins = max(1, int(self.gating_bins))
        if n_bins <= 1:
            return np.zeros(0, dtype=float)
        quantiles = np.linspace(0.0, 1.0, n_bins + 1)[1:-1]
        cuts = np.quantile(np.asarray(x_col, dtype=float), quantiles)
        cuts = np.unique(np.asarray(cuts, dtype=float))
        cuts = cuts[np.isfinite(cuts)]
        return cuts

    def _assign_bins(self, x_col, cuts):
        x_col = np.asarray(x_col, dtype=float).reshape(-1)
        if cuts.size == 0:
            return np.zeros(x_col.shape[0], dtype=int)
        return np.searchsorted(cuts, x_col, side="right")

    def _blend_from_bins(self, preds, x_col, cuts, bin_weights):
        n = preds.shape[0]
        if n == 0:
            return np.zeros(0, dtype=float)
        bin_ids = self._assign_bins(x_col, cuts)
        out = np.empty(n, dtype=float)
        for b in range(bin_weights.shape[0]):
            mask = bin_ids == b
            if np.any(mask):
                out[mask] = preds[mask] @ bin_weights[b]
        return out
    ...
    def _make_teachers(self, n_samples, n_features, seed):
        ...
        rf = RandomForestRegressor(
            ...
            max_depth=(None if self.rf_max_depth is None else int(self.rf_max_depth)),
    ...
    def fit(self, X, y):
        ...
        if n_features == 0:
            ...
            self.hgb_ = None
            self.rf_ = None
            self.gating_feature_idx_ = 0
            self.gating_bin_cuts_ = np.zeros(0, dtype=float)
            self.gating_bin_weights_ = np.array([[0.0, 0.0, 0.0, 1.0]], dtype=float)
            self.gating_bin_counts_ = np.array([int(X.shape[0])], dtype=int)
            self.blend_weights_ = np.array([0.0, 0.0, 0.0, 1.0], dtype=float)
        ...
        n_components = 4  # (gbm, hgb, rf, student)

        self.gbm_, self.hgb_, self.rf_ = self._make_teachers(
            n_samples=n_samples,
        ...
        p_fit = np.column_stack([p_gbm, p_hgb, p_rf, p_lin]).astype(float)

        global_weights = self._solve_nonnegative_weights(p_fit, y)
        self.blend_weights_ = global_weights
        ...
        gate_scores = 0.45 * gbm_imp + 0.25 * hgb_imp + 0.30 * rf_imp
        if np.all(np.isfinite(gate_scores)) and float(np.sum(np.abs(gate_scores))) > 1e-12:
            self.gating_feature_idx_ = int(np.argmax(gate_scores))
        else:
            self.gating_feature_idx_ = 0

        x_gate = X[:, self.gating_feature_idx_]
        cuts = self._compute_bin_cuts(x_gate)
        bin_ids = self._assign_bins(x_gate, cuts)
        n_bins = int(cuts.size + 1)
        min_bin = max(16, int(self.gating_min_bin_samples))
        bin_weights = np.tile(global_weights, (n_bins, 1))
        ...
        for b in range(n_bins):
            ...
            if count < min_bin:
                continue
            local_w = self._solve_nonnegative_weights(p_fit[mask], y[mask])
            shrink = float(self.gating_shrinkage) / (float(self.gating_shrinkage) + float(count))
            merged = (1.0 - shrink) * local_w + shrink * global_weights
            total = float(np.sum(merged))
            if np.isfinite(total) and total > 1e-12:
                bin_weights[b] = merged / total

        self.gating_bin_cuts_ = cuts
        self.gating_bin_weights_ = bin_weights
        self.gating_bin_counts_ = bin_counts
        blend_pred = self._blend_from_bins(
            p_fit,
            x_gate,
            self.gating_bin_cuts_,
            self.gating_bin_weights_,
        )
        ...
        total_counts = int(np.sum(self.gating_bin_counts_))
        if total_counts > 0:
            component_weights = (self.gating_bin_weights_ * self.gating_bin_counts_[:, None]).sum(axis=0) / float(total_counts)
        else:
            component_weights = self.blend_weights_

        feature_importance = (
            float(component_weights[0]) * gbm_imp
            + float(component_weights[1]) * hgb_imp
            + float(component_weights[2]) * rf_imp
            + float(component_weights[3]) * lin_imp
        )
    ...
    def predict(self, X):
        ...
        if self.gbm_ is None or self.hgb_ is None or self.rf_ is None:
            blend = p_lin
        else:
            p_gbm = self.gbm_.predict(X)
            p_hgb = self.hgb_.predict(X)
    ...
    def __str__(self):
        ...
        lines = [
            "Quantile-Gated Teacher Blend Probe Atlas Regressor",
            "Predictive core: ridge student + GBM + HistGB + RF with dominant-feature quantile-gated blending.",
            ...
            f"blend_weights_gbm_hgb_rf_student: {self.blend_weights_[0]:.3f}, {self.blend_weights_[1]:.3f}, {self.blend_weights_[2]:.3f}, {self.blend_weights_[3]:.3f}",
            f"gating_feature: x{int(self.gating_feature_idx_)}",
        ...
        if getattr(self, "gating_bin_cuts_", np.zeros(0, dtype=float)).size > 0:
            cuts = ", ".join(f"{c:.3f}" for c in self.gating_bin_cuts_)
            lines.append(f"gating_bin_edges: {cuts}")
        if getattr(self, "gating_bin_weights_", None) is not None:
            for b, w in enumerate(self.gating_bin_weights_):
                lines.append(
                    f"gating_bin_{b}_weights_gbm_hgb_rf_student: "
...
QuantileGatedTeacherBlendAtlasRegressor.__module__ = "interpretable_regressor"
# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "QuantileGatedTeacherBlendApr18ac"
model_description = (
    "Ridge student + GBM/HistGB/RF teacher blend with dominant-feature quantile-gated local weights, affine calibration, and probe-answer simulation atlas."
)
model_defs = [
    (
        model_shorthand_name,
        QuantileGatedTeacherBlendAtlasRegressor(
            student_alpha_grid=(0.01, 0.04, 0.12, 0.35, 1.0, 3.0),
            student_cv_folds=4,
            student_cv_min_samples=72,
            blend_l2=1e-4,
