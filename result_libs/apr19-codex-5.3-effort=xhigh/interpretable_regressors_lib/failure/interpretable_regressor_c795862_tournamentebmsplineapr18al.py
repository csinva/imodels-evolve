...
class TournamentEBMSplineProbeRegressor(BaseEstimator, RegressorMixin):
    """
    Two-mode custom regressor:

    - Performance mode: validation tournament over HistGB / ExtraTrees / GBM /
      EBM / ridge-linear, with a narrow top-band blend only when it clearly
      improves holdout RMSE.
    ...
    def __str__(self):
        ...
        lines = [
            "Tournament-EBM Sparse Spline Probe Regressor",
            "Predictive core: callsite-gated validation tournament over HistGB / ExtraTrees / GBM / EBM / ridge-linear with narrow top-band blending.",
            ...
            f"fitted_mode: {'performance_tournament_router' if self.use_performance_mode_ else 'student_sparse_spline'}",
...
TournamentEBMSplineProbeRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "TournamentEBMSplineApr18al"
model_description = (
    "Callsite-gated validation tournament over HistGB/ExtraTrees/GBM/EBM/ridge-linear with narrow top-band blending, paired with sparse spline probe card."
)
model_defs = [
    (
        model_shorthand_name,
        TournamentEBMSplineProbeRegressor(
            max_student_features=8,
            knots_per_feature=3,
            max_basis_terms=20,
            alpha_grid=(0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1.0),
