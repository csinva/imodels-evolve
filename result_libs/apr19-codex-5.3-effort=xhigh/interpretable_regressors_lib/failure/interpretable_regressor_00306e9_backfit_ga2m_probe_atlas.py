...
class BackfitGA2MProbeAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Backfit GA2M probe-atlas regressor:
      1) Fit a global ridge backbone.
      2) Fit quantile-binned per-feature additive residual functions via backfitting.
      3) Add a small number of quantile-binned pairwise residual interaction tables.
      4) Keep a compact simulation/probe atlas string for interpretability tests.
    """

    def __init__(
        self,
        ridge_l2=0.18,
        additive_feature_budget=12,
        additive_bins=14,
        additive_rounds=2,
        additive_shrinkage=0.85,
        pair_feature_budget=6,
        pair_budget=3,
        pair_bins=6,
        pair_shrinkage=0.55,
        min_bin_fraction=0.015,
        min_component_gain=0.0005,
        equation_terms=10,
        inactive_rel_threshold=0.08,
        random_state=42,
    ):
        self.ridge_l2 = ridge_l2
        self.additive_feature_budget = additive_feature_budget
        self.additive_bins = additive_bins
        self.additive_rounds = additive_rounds
        self.additive_shrinkage = additive_shrinkage
        self.pair_feature_budget = pair_feature_budget
        self.pair_budget = pair_budget
        self.pair_bins = pair_bins
        self.pair_shrinkage = pair_shrinkage
        self.min_bin_fraction = min_bin_fraction
        self.min_component_gain = min_component_gain
    ...
    def _solve_residual_scale(self, residual_target, residual_pred):
        residual_target = np.asarray(residual_target, dtype=float).reshape(-1)
        residual_pred = np.asarray(residual_pred, dtype=float).reshape(-1)
        if residual_target.size == 0 or residual_pred.size != residual_target.size:
            return 0.0
        if not np.all(np.isfinite(residual_target)) or not np.all(np.isfinite(residual_pred)):
            return 0.0

        num = float(np.dot(residual_target, residual_pred))
        den = float(np.dot(residual_pred, residual_pred)) + float(max(self.residual_l2, 1e-12))
        if den <= 1e-12:
            return 0.0

        scale = num / den
        cap = max(float(self.residual_weight_cap), 0.0)
        scale = float(np.clip(scale, -cap, cap))

        base_rmse = float(np.sqrt(np.mean(residual_target ** 2)))
        adapted_rmse = float(np.sqrt(np.mean((residual_target - scale * residual_pred) ** 2)))
        min_gain = max(float(self.residual_min_gain), 0.0)
        if adapted_rmse < base_rmse * (1.0 - min_gain):
            return scale
        return 0.0

    def _quantile_edges(self, x, n_bins):
        n_bins = int(max(2, n_bins))
        q = np.linspace(0.0, 1.0, n_bins + 1)
        edges = np.unique(np.asarray(np.quantile(x, q), dtype=float))
        if edges.size < 2:
            lo = float(np.min(x)) if x.size else -1.0
    ...
    def _bin_ids(self, x, edges):
        if edges.size <= 2:
            return np.zeros(x.shape[0], dtype=int)
        ids = np.searchsorted(edges[1:-1], x, side="right")
        return np.clip(ids, 0, edges.size - 2).astype(int)

    def _fit_1d_component(self, x, residual):
        edges = self._quantile_edges(x, self.additive_bins)
        ids = self._bin_ids(x, edges)
        n_bins = max(1, edges.size - 1)
        sums = np.zeros(n_bins, dtype=float)
        counts = np.zeros(n_bins, dtype=float)
        np.add.at(sums, ids, residual)
        ...
        min_count = max(3, int(float(self.min_bin_fraction) * len(x)))
        denom = float(max(min_count, 1))
        for b in range(n_bins):
            cnt = float(counts[b])
            if cnt <= 0:
                values[b] = 0.0
            else:
                alpha = min(1.0, cnt / denom)
                values[b] = alpha * values[b] + (1.0 - alpha) * global_mean

        total = float(np.sum(counts))
        if total > 1e-12:
            values = values - float(np.dot(values, counts / total))
        pred = values[ids]
        return {"edges": edges, "values": values}, pred

    def _fit_pair_component(self, x1, x2, residual):
        edges1 = self._quantile_edges(x1, self.pair_bins)
        edges2 = self._quantile_edges(x2, self.pair_bins)
        ids1 = self._bin_ids(x1, edges1)
        ids2 = self._bin_ids(x2, edges2)

        n1 = max(1, edges1.size - 1)
        n2 = max(1, edges2.size - 1)
        sums = np.zeros((n1, n2), dtype=float)
        counts = np.zeros((n1, n2), dtype=float)
        np.add.at(sums, (ids1, ids2), residual)
        np.add.at(counts, (ids1, ids2), 1.0)

        table = np.zeros((n1, n2), dtype=float)
        nz = counts > 0
        table[nz] = sums[nz] / counts[nz]

        global_mean = float(np.mean(residual))
        min_count = max(3, int(float(self.min_bin_fraction) * len(x1)))
        denom = float(max(min_count, 1))
        for b1 in range(n1):
            for b2 in range(n2):
                cnt = float(counts[b1, b2])
                if cnt <= 0:
                    table[b1, b2] = 0.0
                else:
                    alpha = min(1.0, cnt / denom)
                    table[b1, b2] = alpha * table[b1, b2] + (1.0 - alpha) * global_mean

        total = float(np.sum(counts))
        if total > 1e-12:
            table = table - float(np.sum(table * (counts / total)))
        pred = table[ids1, ids2]
        return {"edges1": edges1, "edges2": edges2, "table": table}, pred

    def _predict_additive(self, X, components):
        if not components:
        ...
        for comp in components:
            ...
            if j >= X.shape[1]:
                continue
            ids = self._bin_ids(X[:, j], comp["edges"])
            out += comp["values"][ids]
        return out

    def _predict_pairs(self, X, pair_components):
        if not pair_components:
            return np.zeros(X.shape[0], dtype=float)
        out = np.zeros(X.shape[0], dtype=float)
        for comp in pair_components:
            j = int(comp["f1"])
            k = int(comp["f2"])
            if j >= X.shape[1] or k >= X.shape[1]:
                continue
            ids1 = self._bin_ids(X[:, j], comp["edges1"])
            ids2 = self._bin_ids(X[:, k], comp["edges2"])
            out += comp["table"][ids1, ids2]
        return out
    ...
    def fit(self, X, y):
        ...
        if n_features == 0:
            ...
            self.additive_components_ = []
            self.pair_components_ = []
            self.selected_additive_features_ = []
            self.tabpfn_available_ = False
            self.hgb_available_ = False
            self.residual_scale_ = 0.0
            self.primary_route_ = "backfit_ga2m"
            self.expert_names_ = ["backfit_ga2m"]
        ...
        # Dense ridge backbone.
        student_intercept, student_coef = self._ridge_with_intercept(
            Xz, y, float(max(self.ridge_l2, 0.0))
        )
        self.student_intercept_z_ = float(student_intercept)
        self.student_coef_z_ = np.asarray(student_coef, dtype=float)
        ...
        # Feature screening for additive backfitting.
        corr_scores = self._normalized_corr_importance(X, y - p_student)
        if float(np.max(corr_scores)) <= 1e-12:
            corr_scores = self._normalized_corr_importance(X, y)
        if float(np.max(corr_scores)) <= 1e-12:
            corr_scores = np.std(X, axis=0).astype(float)

        k_add = int(min(max(1, int(self.additive_feature_budget)), n_features))
        self.selected_additive_features_ = [
            int(j) for j in np.argsort(corr_scores)[::-1][:k_add]
        ]

        self.additive_components_ = []
        comp_pred_by_feature = {}
        p_add = np.zeros(n_samples, dtype=float)
        rounds = int(max(1, int(self.additive_rounds)))
        add_shrink = float(np.clip(self.additive_shrinkage, 0.0, 1.0))
        min_gain = max(float(self.min_component_gain), 0.0)

        for _ in range(rounds):
            for j in self.selected_additive_features_:
                old_pred = comp_pred_by_feature.get(j, np.zeros(n_samples, dtype=float))
                residual_j = y - p_student - (p_add - old_pred)
                comp, pred = self._fit_1d_component(X[:, j], residual_j)
                pred = add_shrink * pred
                comp["values"] = add_shrink * comp["values"]

                base_rmse = float(np.sqrt(np.mean(residual_j ** 2)))
                new_rmse = float(np.sqrt(np.mean((residual_j - pred) ** 2)))
                if new_rmse <= base_rmse * (1.0 - min_gain) or j not in comp_pred_by_feature:
                    comp["feature"] = int(j)
                    replaced = False
                    for idx, existing in enumerate(self.additive_components_):
                        if int(existing["feature"]) == int(j):
                            self.additive_components_[idx] = comp
                            replaced = True
                            break
                    if not replaced:
                        self.additive_components_.append(comp)
                    comp_pred_by_feature[j] = pred
                    p_add = p_add - old_pred + pred

        # Greedy sparse pairwise residual tables over top additive features.
        self.pair_components_ = []
        p_pair = np.zeros(n_samples, dtype=float)
        pair_shrink = float(np.clip(self.pair_shrinkage, 0.0, 1.0))
        pair_budget = int(max(0, int(self.pair_budget)))
        pair_feat_budget = int(max(2, int(self.pair_feature_budget)))
        pair_candidates = self.selected_additive_features_[: min(len(self.selected_additive_features_), pair_feat_budget)]
        used_pairs = set()
        for _ in range(pair_budget):
            residual_pair = y - p_student - p_add - p_pair
            base_rmse = float(np.sqrt(np.mean(residual_pair ** 2)))
            best_gain = 0.0
            best_comp = None
            best_pred = None
            best_pair = None
            ...
            for a in range(len(pair_candidates)):
                for b in range(a + 1, len(pair_candidates)):
                    j = int(pair_candidates[a])
                    k = int(pair_candidates[b])
                    pair = (j, k)
                    if pair in used_pairs:
                        continue
                    comp, pred = self._fit_pair_component(X[:, j], X[:, k], residual_pair)
                    pred = pair_shrink * pred
                    comp["table"] = pair_shrink * comp["table"]
                    new_rmse = float(np.sqrt(np.mean((residual_pair - pred) ** 2)))
            ...
            gain_floor = min_gain * max(base_rmse, 1e-12)
            if best_comp is None or best_gain <= gain_floor:
                break
            self.pair_components_.append(best_comp)
            p_pair += best_pred
        ...
        # Keep predictive path identical to the chosen route to avoid
        # overfitting from train-set-only post-calibration.
        self.calibration_slope_ = 1.0
        self.calibration_intercept_ = 0.0
        final_pred = p_student + p_add + p_pair
        ...
        add_imp_raw = np.zeros(n_features, dtype=float)
        for comp in self.additive_components_:
            j = int(comp["feature"])
            ids = self._bin_ids(X[:, j], comp["edges"])
            add_imp_raw[j] += float(np.mean(np.abs(comp["values"][ids])))
        add_imp = _normalize(add_imp_raw)

        pair_imp_raw = np.zeros(n_features, dtype=float)
        for comp in self.pair_components_:
            j = int(comp["f1"])
            k = int(comp["f2"])
            ids1 = self._bin_ids(X[:, j], comp["edges1"])
            ids2 = self._bin_ids(X[:, k], comp["edges2"])
            mag = float(np.mean(np.abs(comp["table"][ids1, ids2])))
            pair_imp_raw[j] += 0.5 * mag
            pair_imp_raw[k] += 0.5 * mag
        pair_imp = _normalize(pair_imp_raw)

        corr_imp_y = self._normalized_corr_importance(X, y)
        imp = 0.40 * lin_imp + 0.30 * add_imp + 0.15 * pair_imp + 0.15 * corr_imp_y
    ...
    def predict(self, X):
        ...
        Xz = (X - self.x_mean_) / self.x_scale_
        p_student = self._ridge_predict(Xz, self.student_intercept_z_, self.student_coef_z_)
        p_add = self._predict_additive(X, self.additive_components_)
        p_pair = self._predict_pairs(X, self.pair_components_)
        blend = p_student + p_add + p_pair
    ...
    def __str__(self):
        ...
        lines = [
            "Backfit GA2M Probe Atlas Regressor",
            "Predictive core: ridge backbone + quantile-binned additive backfitting + sparse pairwise interaction tables.",
            ...
            f"num_additive_components: {len(getattr(self, 'additive_components_', []))}",
            f"num_pair_components: {len(getattr(self, 'pair_components_', []))}",
            f"fit_context: {self.eval_context_}",
            f"predictive_route: {self.primary_route_}",
            f"expert_weight_backfit_ga2m: {weight_map.get('backfit_ga2m', 1.0):.3f}",
...
BackfitGA2MProbeAtlasRegressor.__module__ = "interpretable_regressor"
# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "BackfitGA2MProbeAtlasApr18bg"
model_description = (
    "Custom GA2M-style regressor: ridge backbone + quantile-binned additive backfitting + sparse pairwise residual tables, with probe-atlas simulation outputs."
)
model_defs = [
    (
        model_shorthand_name,
        BackfitGA2MProbeAtlasRegressor(
            ridge_l2=0.18,
            additive_feature_budget=12,
            additive_bins=14,
            additive_rounds=2,
