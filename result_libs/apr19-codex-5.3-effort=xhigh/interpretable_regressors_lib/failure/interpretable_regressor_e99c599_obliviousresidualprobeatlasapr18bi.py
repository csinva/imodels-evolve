...
class ObliviousResidualProbeAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Custom interpretable regressor with two components:
      1) Ridge linear backbone on standardized features.
      2) Boosted oblivious residual trees (same split at each depth level).

    The tree booster is implemented from scratch with a split criterion that
    combines residual fit and split-balance regularization.
    """
    ...
    def fit(self, X, y):
        ...
        pred = np.array(pred_linear, dtype=float)

        thresholds_by_feature = [self._feature_thresholds(X[:, j]) for j in range(n_features)]
        ...
        max_rounds = int(
            min(
                max(int(self.n_rounds + self.round_scale * np.sqrt(max(n_features, 1))), 0),
                int(max(self.max_rounds_cap, 1)),
            )
        )
        ...
        for _ in range(max_rounds):
            ...
            corr_scores = np.array([self._centered_abs_corr(X[:, j], residual) for j in range(n_features)], dtype=float)
            screen = int(min(max(2, int(self.feature_screen)), n_features))
            cand = np.argsort(corr_scores)[::-1][:screen].tolist()

            if float(self.feature_subsample) < 0.999 and len(cand) > 2:
                k = int(max(2, round(float(self.feature_subsample) * len(cand))))
                sel = rng.choice(len(cand), size=min(k, len(cand)), replace=False)
                cand = [cand[i] for i in np.sort(sel)]

            tree = self._build_one_oblivious_tree(
                X=X,
                residual=residual,
            ...
            if tree is None:
                no_improve_rounds += 1
                if no_improve_rounds >= int(max(self.boost_patience, 1)):
                    break
                continue

            update = self._tree_predict(X, tree)
            pred += update
            trees.append(tree)
            no_improve_rounds = 0

            for (j, _), g in zip(tree["splits"], tree["split_gains"]):
                tree_feature_gain[int(j)] += float(max(g, 0.0))

        self.oblivious_trees_ = trees
        self.oblivious_tree_count_ = len(trees)
        self.oblivious_avg_depth_ = float(np.mean([len(t["splits"]) for t in trees])) if trees else 0.0
        self.tree_feature_gain_ = tree_feature_gain

        final_pred = np.array(pred, dtype=float)
        ...
        tree_imp = _normalize(self.tree_feature_gain_)
        corr_imp_y = self._normalized_corr_importance(X, y)

        if np.max(tree_imp) > 1e-12:
            imp = 0.45 * tree_imp + 0.35 * lin_imp + 0.20 * corr_imp_y
        else:
        ...
        boost_pred = final_pred - pred_linear
    ...
    def predict(self, X):
        ...
        for tree in self.oblivious_trees_:
            pred += self._tree_predict(X, tree)
    ...
    def __str__(self):
        ...
        lines = [
            "Residual Oblivious Tree Atlas Regressor",
            "Predictive core: ridge backbone plus boosted oblivious residual trees.",
...
ObliviousResidualProbeAtlasRegressor.__module__ = "interpretable_regressor"
# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "ObliviousResidualProbeAtlasApr18bi"
model_description = (
    "Custom ridge backbone plus boosted oblivious residual trees with a split-balance criterion, exposed through a probe-answer simulation atlas."
)
model_defs = [
    (
        model_shorthand_name,
        ObliviousResidualProbeAtlasRegressor(
            ridge_l2=0.22,
            n_rounds=72,
            depth=3,
            learning_rate=0.14,
