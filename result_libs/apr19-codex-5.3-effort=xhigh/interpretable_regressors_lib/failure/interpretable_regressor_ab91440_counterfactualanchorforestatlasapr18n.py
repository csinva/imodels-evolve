...
class CounterfactualAnchorForestAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Five-stage custom regressor:
      ...
      4) Lightweight residual boosting with axis-aligned stumps.
      5) Feature-bagged residual tree bank with ridge-stacked tree weights.
    ...
    def __init__(
        self,
        ridge_l2=0.55,
        ridge_scale_with_dim=0.32,
        spline_feature_budget=18,
        spline_quantiles=(0.08, 0.18, 0.32, 0.5, 0.68, 0.82, 0.92),
        interaction_screen=12,
        interaction_budget=16,
        n_stumps=36,
        stump_count_scale=0.5,
        max_stumps_cap=64,
        learning_rate=0.16,
        feature_scan=20,
        ...
        n_residual_trees=28,
        tree_count_scale=1.3,
        max_residual_trees_cap=64,
        tree_max_depth=4,
        tree_min_leaf=10,
        tree_feature_fraction=0.55,
        tree_row_fraction=0.8,
        tree_weight_l2=0.24,
        tree_keep_fraction=0.65,
        random_state=42,
    ):
        ...
        self.stump_count_scale = stump_count_scale
        self.max_stumps_cap = max_stumps_cap
        ...
        self.max_residual_trees_cap = max_residual_trees_cap
        ...
        self.tree_keep_fraction = tree_keep_fraction
    ...
    def fit(self, X, y):
        ...
        if n_features > 0:
            q_stump = np.asarray(self.quantile_grid, dtype=float)
            for j in range(n_features):
                xj = X[:, j]
                vals = np.quantile(xj, q_stump) if q_stump.size > 0 else np.array([], dtype=float)
                vals = np.concatenate(([0.0], np.asarray(vals, dtype=float)))
                thresholds_by_feature[int(j)] = np.unique(np.round(vals, 6))
        ...
        stump_rounds = int(
            min(
                max(int(self.n_stumps + self.stump_count_scale * np.sqrt(max(n_features, 1))), 0),
                int(self.max_stumps_cap),
            )
        )
        ...
        # Feature-bagged residual tree bank, then ridge-stack tree outputs.
        tree_bank = []
        tree_feature_ids = []
        tree_weights = np.zeros(0, dtype=float)
        tree_feature_gain = np.zeros(n_features, dtype=float)
        if n_features > 0 and int(self.n_residual_trees) > 0:
            rng = np.random.RandomState(self.random_state)
            residual_tree_target = y - (linear_pred + stump_pred)
            tree_count = int(
                min(
                    max(int(self.n_residual_trees + self.tree_count_scale * np.sqrt(max(n_features, 1))), 0),
                    int(self.max_residual_trees_cap),
                )
            )
            feat_budget = int(
                min(
            ...
            for _ in range(tree_count):
                ...
                row_idx = rng.choice(n_samples, size=row_budget, replace=True).astype(int)
                ...
                tree.fit(X[row_idx][:, feat_idx], residual_tree_target[row_idx])
                pred_col = tree.predict(X[:, feat_idx]).astype(float)
                tree_cols.append(pred_col)
                tree_bank.append(tree)
                tree_feature_ids.append(feat_idx)

            if tree_cols:
                Z = np.column_stack(tree_cols).astype(float)
                l2 = float(self.tree_weight_l2) * (1.0 + 0.5 * Z.shape[1] / max(n_samples, 1))
                lhs = Z.T @ Z + np.eye(Z.shape[1], dtype=float) * l2
                rhs = Z.T @ residual_tree_target
                try:
                    tree_weights = np.linalg.solve(lhs, rhs)
                except np.linalg.LinAlgError:
                    tree_weights = np.linalg.lstsq(lhs, rhs, rcond=None)[0]
                tree_weights = np.asarray(tree_weights, dtype=float)

                keep_frac = float(np.clip(self.tree_keep_fraction, 0.0, 1.0))
                if keep_frac < 1.0 and tree_weights.size > 0:
                    keep = int(max(1, np.ceil(keep_frac * tree_weights.size)))
                    keep_idx = np.argsort(np.abs(tree_weights))[::-1][:keep]
                    keep_mask = np.zeros(tree_weights.size, dtype=bool)
                    keep_mask[keep_idx] = True
                    tree_weights = np.where(keep_mask, tree_weights, 0.0)

                for feat_idx, w in zip(tree_feature_ids, tree_weights):
                    w_abs = abs(float(w))
                    if w_abs <= 0.0:
                        continue
    ...
    def __str__(self):
        ...
        lines = [
            "Counterfactual Anchor Forest Atlas Regressor",
...
CounterfactualAnchorForestAtlasRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "CounterfactualAnchorForestAtlasApr18n"
model_description = (
    "Adaptive-L2 spline/interactions backbone plus feature-bagged residual tree bank with ridge-stacked tree weights and explicit probe-answer anchors."
)
model_defs = [
    (
        model_shorthand_name,
        CounterfactualAnchorForestAtlasRegressor(
            ridge_l2=0.55,
            ridge_scale_with_dim=0.32,
            spline_feature_budget=18,
            spline_quantiles=(0.08, 0.18, 0.32, 0.5, 0.68, 0.82, 0.92),
