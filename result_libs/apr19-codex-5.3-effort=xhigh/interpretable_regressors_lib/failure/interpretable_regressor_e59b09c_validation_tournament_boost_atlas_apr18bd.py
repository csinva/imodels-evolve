...
class ValidationTournamentBoostAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Validation-tournament boosted stacker:
      1) Standardized ridge student (compact equation path).
      2) Primary GradientBoosting expert on raw features.
      3) HistGradientBoosting residual adapter (optional).
      4) Quantile residual table (optional).
      5) Holdout tournament: keep the convex blend only if it beats the best single expert.
    ...
    def __init__(
        ...
        blend_l2=1e-3,
        min_component_weights=(0.0, 0.0, 0.0),  # (hybrid, primary, student)
        blend_prior=(0.0, 0.0, 0.0),
        blend_prior_strength=0.0,
        blend_min_improvement=0.01,
        gbm_n_estimators=120,
        gbm_learning_rate=0.08,
        gbm_max_depth=3,
        gbm_min_samples_leaf=3,
        gbm_subsample=1.00,
        hgb_max_iter_base=90,
        hgb_max_iter_scale=1.2,
        hgb_max_iter_cap=180,
        hgb_learning_rate=0.06,
        hgb_max_leaf_nodes=31,
        hgb_min_samples_leaf=12,
        hgb_l2_regularization=5e-3,
        residual_quantile_features=0,
        residual_quantiles=(0.15, 0.35, 0.50, 0.65, 0.85),
        residual_quantile_l2=5e-2,
        ...
        self.blend_min_improvement = blend_min_improvement
    ...
    def fit(self, X, y):
        ...
        use_val = val_idx is not None and len(val_idx) >= int(self.min_validation_samples)
        pred_for_select = pred_matrix[val_idx] if use_val else pred_matrix
        y_for_select = y[val_idx] if use_val else y

        weights_fit = self._solve_nonnegative_weights(pred_for_select, y_for_select)
        weights_fit = np.asarray(weights_fit, dtype=float) * availability

        if float(np.sum(weights_fit)) > 1e-12:
            weights_fit = weights_fit / float(np.sum(weights_fit))
        else:
            weights_fit = np.array([0.0, 0.0, 1.0], dtype=float)

        single_rmses = []
        for j in range(pred_for_select.shape[1]):
            if availability[j] <= 0.0:
                single_rmses.append(np.inf)
                continue
            rmse_j = float(np.sqrt(np.mean((y_for_select - pred_for_select[:, j]) ** 2)))
            single_rmses.append(rmse_j)
        best_single_idx = int(np.argmin(single_rmses))
        best_single_rmse = float(single_rmses[best_single_idx])

        blend_pred_select = pred_for_select @ weights_fit
        blend_rmse = float(np.sqrt(np.mean((y_for_select - blend_pred_select) ** 2)))

        improvement_needed = max(1e-6, float(self.blend_min_improvement))
        if np.isfinite(blend_rmse) and blend_rmse < best_single_rmse * (1.0 - improvement_needed):
            weights = weights_fit
        else:
        ...
        self.blend_weights_ = np.asarray(weights, dtype=float)
        ...
        if use_val:
            blend_for_cal = (
                float(self.blend_weights_[0]) * p_hybrid_plus_quantile[val_idx]
                + float(self.blend_weights_[1]) * p_primary[val_idx]
                + float(self.blend_weights_[2]) * p_lin[val_idx]
            )
            y_for_cal = y[val_idx]
        else:
            blend_for_cal = blend_pred
            y_for_cal = y

        blend_centered = blend_for_cal - float(np.mean(blend_for_cal))
        y_centered = y_for_cal - float(np.mean(y_for_cal))
        ...
        intercept = float(np.mean(y_for_cal) - slope * np.mean(blend_for_cal))
    ...
    def _solve_for_x0(self, target, fixed, lo=-10.0, hi=10.0):
        ...
        sign_change = np.where(diffs[:-1] * diffs[1:] <= 0.0)[0]
        if sign_change.size == 0:
            return float(grid[int(np.argmin(np.abs(diffs)))])

        best_idx = int(
            sign_change[
                np.argmin(np.abs(diffs[sign_change]) + np.abs(diffs[sign_change + 1]))
            ]
        )
        left = float(grid[best_idx])
        right = float(grid[best_idx + 1])
        f_left = float(diffs[best_idx])
        f_right = float(diffs[best_idx + 1])

        for _ in range(60):
            mid = 0.5 * (left + right)
            assign = dict(fixed)
            assign[0] = mid
            f_mid_pred = self._predict_probe(assign)
            if f_mid_pred is None or not np.isfinite(f_mid_pred):
                return None
            f_mid = float(f_mid_pred) - target
            if f_left * f_mid <= 0.0:
                right = mid
                f_right = f_mid
            else:
    ...
    def __str__(self):
        ...
        lines = [
            "Validation Tournament Boost Atlas Regressor",
            "Predictive core: gradient boosting expert + optional residual histogram/quantile adapters + ridge student, with holdout tournament selection of blend vs best single expert.",
...
ValidationTournamentBoostAtlasRegressor.__module__ = "interpretable_regressor"
# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "ValidationTournamentBoostAtlasApr18bd"
model_description = (
    "Holdout-tournament boosted stacker: ridge student + GBM core with optional HistGB/quantile residual adapters, using convex blending only when validation RMSE beats the best single expert."
)
model_defs = [
    (
        model_shorthand_name,
        ValidationTournamentBoostAtlasRegressor(
            student_alpha_grid=(0.001, 0.005, 0.02, 0.08, 0.3, 1.0),
            validation_fraction=0.20,
            min_validation_samples=40,
            blend_l2=1e-3,
            min_component_weights=(0.0, 0.0, 0.0),
            blend_prior=(0.0, 0.0, 0.0),
            blend_prior_strength=0.0,
            blend_min_improvement=0.01,
            gbm_n_estimators=120,
