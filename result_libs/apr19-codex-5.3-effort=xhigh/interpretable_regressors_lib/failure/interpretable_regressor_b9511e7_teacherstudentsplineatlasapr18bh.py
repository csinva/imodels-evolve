...
class TeacherStudentSplineAtlasRegressor(BaseEstimator, RegressorMixin):
    """
    Teacher-student spline-atlas regressor:
      1) Fit a global ridge student on standardized features.
      2) Fit a gradient-boosting teacher on raw features.
      3) Learn a validation-based nonnegative blend between teacher and student.
      4) Expose a compact simulation/probe atlas in __str__ for interpretability tests.
    """
    ...
    def fit(self, X, y):
        ...
        if n_features == 0:
            ...
            self.blend_weights_ = np.array([0.0, 1.0], dtype=float)
            ...
            self.primary_route_ = "teacher_student_blend"
            self.expert_names_ = ["teacher_gbm", "student_ridge"]
        ...
        def _make_teacher(seed):
            n_estimators = int(np.clip(80.0 + 3.0 * np.sqrt(max(n_samples, 1)), 90, 220))
            max_depth = 3 if n_features >= 5 else 2
            min_leaf = int(np.clip(max(2.0, 0.01 * n_samples), 2, 40))
            max_features = float(np.clip(18.0 / max(n_features, 1), 0.4, 1.0))
        ...
        if n_samples >= 60:
            ...
            tr_intercept, tr_coef = self._ridge_with_intercept(Xtr_z, ytr, l2)
            p_student_val = self._ridge_predict(Xv_z, tr_intercept, tr_coef)

            teacher_val_model = None
            try:
                teacher_val_model = _make_teacher(int(self.random_state) + 1)
                teacher_val_model.fit(Xtr, ytr)
            except Exception:
                teacher_val_model = None

            if teacher_val_model is not None:
                p_teacher_val = self._safe_predict(teacher_val_model, Xv, p_student_val)
                delta = p_teacher_val - p_student_val
                den = float(np.dot(delta, delta))
                if np.isfinite(den) and den > 1e-12:
                    num = float(np.dot(yv - p_student_val, delta))
                    w_teacher = num / den
                w_teacher = float(np.clip(w_teacher, 0.0, 1.0))
                # Keep a mild teacher prior so the blend stays performance-first.
                w_teacher = float(np.clip(0.75 * w_teacher + 0.25 * 0.85, 0.0, 1.0))

        self.student_intercept_z_, self.student_coef_z_ = self._ridge_with_intercept(Xz, y, l2)
        self.student_intercept_z_ = float(self.student_intercept_z_)
        self.student_coef_z_ = np.asarray(self.student_coef_z_, dtype=float)
        p_student = self._ridge_predict(Xz, self.student_intercept_z_, self.student_coef_z_)

        self.teacher_model_ = None
        try:
            self.teacher_model_ = _make_teacher(int(self.random_state) + 17)
            self.teacher_model_.fit(X, y)
        except Exception:
            self.teacher_model_ = None

        if self.teacher_model_ is None:
            w_teacher = 0.0
        self.blend_weights_ = np.array([float(w_teacher), float(1.0 - w_teacher)], dtype=float)
        if float(np.sum(self.blend_weights_)) <= 1e-12:
            self.blend_weights_ = np.array([0.0, 1.0], dtype=float)
        self.blend_weights_ = self.blend_weights_ / float(np.sum(self.blend_weights_))

        p_teacher = self._safe_predict(self.teacher_model_, X, p_student)
        final_pred = self.blend_weights_[0] * p_teacher + self.blend_weights_[1] * p_student
        ...
        tree_imp = _normalize(self._safe_feature_importances(self.teacher_model_, n_features))
        corr_imp_y = self._normalized_corr_importance(X, y)
        if np.max(tree_imp) > 1e-12:
            imp = 0.55 * tree_imp + 0.30 * lin_imp + 0.15 * corr_imp_y
        else:
    ...
    def predict(self, X):
        ...
        p_student = self._ridge_predict(Xz, self.student_intercept_z_, self.student_coef_z_)
        p_teacher = self._safe_predict(self.teacher_model_, X, p_student)
        blend = self.blend_weights_[0] * p_teacher + self.blend_weights_[1] * p_student
    ...
    def __str__(self):
        ...
        lines = [
            "Teacher-Student Spline Atlas Regressor",
            "Predictive core: validation-weighted blend of GBM teacher and ridge student.",
            ...
            f"expert_weight_teacher_gbm: {weight_map.get('teacher_gbm', 0.0):.3f}",
            f"expert_weight_student_ridge: {weight_map.get('student_ridge', 1.0):.3f}",
...
TeacherStudentSplineAtlasRegressor.__module__ = "interpretable_regressor"
# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "TeacherStudentSplineAtlasApr18bh"
model_description = (
    "Validation-blended GBM teacher + ridge student with a compact probe-answer simulation atlas."
)
model_defs = [
    (
        model_shorthand_name,
        TeacherStudentSplineAtlasRegressor(
            ridge_l2=0.12,
            equation_terms=8,
            inactive_rel_threshold=0.08,
            random_state=42,
