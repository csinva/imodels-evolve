...
class CrossFitHistSplineLedgerRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.hist_model_ is not None and self.weight_hist_ > 0.5:
            self.teacher_family_ = "hist_primary"
        elif self.hist_model_ is not None:
            self.teacher_family_ = "hist_blend"
        else:
    ...
    def __str__(self):
        ...
        lines = [
            "Cross-Fit Hist Spline Ledger Regressor",
            "Predictive core: HistGradientBoosting teacher blended with sparse spline student and ridge anchor.",
...
model_shorthand_name = "CrossFitHistSplineLedgerApr19g"
model_description = (
    "Custom teacher-student regressor: HistGradientBoosting teacher blended via holdout simplex weights with a sparse handcrafted hinge/interaction student and ridge anchor, plus probe-ledger simulation card."
)
model_defs = [
    (
