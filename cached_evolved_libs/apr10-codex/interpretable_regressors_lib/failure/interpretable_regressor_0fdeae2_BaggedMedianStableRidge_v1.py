...
class BaggedMedianStableRidgeRegressor(BaseEstimator, RegressorMixin):
    """
    Stability-focused sparse linear regressor:
    1) choose ridge alpha by GCV on standardized features,
    2) fit many bootstrap ridge models,
    3) keep features with high median-magnitude and sign consistency,
    4) refit ridge on retained features for final equation.
    """
    ...
    @staticmethod
    def _ridge_from_centered(Xc, yc, alpha):
        if Xc.shape[1] == 0:
            return np.zeros(0, dtype=float)
        A = Xc.T @ Xc + float(alpha) * np.eye(Xc.shape[1], dtype=float)
        b = Xc.T @ yc
    ...
    def _fit_ridge_standardized(self, Xs, y, alpha):
        x_mean = np.mean(Xs, axis=0)
        y_mean = float(np.mean(y))
        Xc = Xs - x_mean
        yc = y - y_mean
        coef = self._ridge_from_centered(Xc, yc, alpha)
    ...
    def fit(self, X, y):
        ...
        for b in range(n_boot):
            ...
            _, coef_b = self._fit_ridge_standardized(Xs[idx], y[idx], alpha)
            coef_boot[b] = coef_b

        coef_med = np.median(coef_boot, axis=0)
        sign_consistency = np.abs(np.mean(np.sign(coef_boot), axis=0))
        score = np.abs(coef_med) * sign_consistency

        kmin = int(max(1, min(int(self.min_features), p)))
        kmax = int(max(kmin, min(int(self.max_features), p)))
        thresh = float(np.quantile(score, float(np.clip(self.score_quantile, 0.0, 1.0))))
        active = np.where(score >= thresh)[0].tolist()

        if len(active) < kmin:
            active = np.argsort(-score)[:kmin].tolist()
        if len(active) > kmax:
            active = np.argsort(-score)[:kmax].tolist()
        active = sorted(int(i) for i in active)

        X_act = Xs[:, active] if active else np.zeros((n, 0), dtype=float)
        intercept_std, coef_act = self._fit_ridge_standardized(X_act, y, alpha)

        coef_std = np.zeros(p, dtype=float)
        for j_local, j_global in enumerate(active):
            coef_std[j_global] = float(coef_act[j_local])

        coef_raw = coef_std / sigma
        intercept_raw = float(intercept_std - np.dot(coef_std, mu / sigma))
    ...
    def __str__(self):
        check_is_fitted(self, "is_fitted_")
        lines = ["Bagged Median-Stable Ridge Regressor:"]
        ...
        for j, c in enumerate(self.coef_):
            if abs(float(c)) >= tol:
                terms.append((abs(float(c)), int(j), float(c), float(self.stability_score_[j])))
        terms.sort(reverse=True)

        if not terms:
            lines.append("  linear terms: none above display threshold")
        else:
            lines.append("  linear terms (sorted by absolute coefficient):")
            for _, j, c, s in terms[:16]:
                lines.append(f"    {c:+.4f} * x{j}   [stability {s:.3f}]")
        return "\n".join(lines)
...
BaggedMedianStableRidgeRegressor.__module__ = "interpretable_regressor"

# Update the model shorthand name and description below to reflect the class above and any changes you make to it.
# The shorthand name should be unique across all experiments (it is used to identify rows in the results CSV files)
# The description should briefly summarize what this experiment tried.
model_shorthand_name = "BaggedMedianStableRidge_v1"
model_description = "Stability-selected sparse ridge using bootstrap median coefficients and sign-consistency scoring before ridge refit"
model_defs = [(model_shorthand_name, BaggedMedianStableRidgeRegressor())]
